...
import { openStorageJournalDb } from "../storage-journal-db";
import { SqliteMigrationRunsRepo, SqliteRestorePointsRepo, SqliteStorageLedgerRepo } from "../storage-journal-repo";

/**
 * @file ADR-041 §2/§4 — integration tests for the sidecar `ops/storage-journal.db` schema +
 ...
 * `features/storage`'s pure-logic layer) — these tests are this session's own coverage for the
 * new schema/adapter layer, per the dispatch's own instruction to write them.
 */

function openTempJournal() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "storage-journal-"));
  const filePath = path.join(tmpDir, "storage-journal.db");
  const db = openStorageJournalDb(filePath);
  return { db, tmpDir };
}

test("migration runs clean: all 3 tables exist after openStorageJournalDb", async () => {
  const { db, tmpDir } = openTempJournal();
  try {
    const ledger = new SqliteStorageLedgerRepo({ db, siteId: "site-1" });
...
test("SqliteStorageLedgerRepo: append + query round-trips a row and filters by kind/outcome", async () => {
  const { db, tmpDir } = openTempJournal();
  try {
    const ledger = new SqliteStorageLedgerRepo({ db, siteId: "site-1" });
...
test("SqliteStorageLedgerRepo: query() paginates via a stable cursor, never a raw offset", async () => {
  const { db, tmpDir } = openTempJournal();
  try {
    const ledger = new SqliteStorageLedgerRepo({ db, siteId: "site-1" });
    for (let i = 0; i < 5; i += 1) {
...
test("SqliteStorageLedgerRepo.appendInterruptedRow: writes a migration.interrupted row (ADR-041 §3 boot reconciliation)", async () => {
  const { db, tmpDir } = openTempJournal();
  try {
    const ledger = new SqliteStorageLedgerRepo({ db, siteId: "site-1" });
...
test("SqliteStorageLedgerRepo: rows are strictly site-scoped — a different siteId never sees another site's rows", async () => {
  const { db, tmpDir } = openTempJournal();
  try {
    const siteA = new SqliteStorageLedgerRepo({ db, siteId: "site-a" });
    const siteB = new SqliteStorageLedgerRepo({ db, siteId: "site-b" });
