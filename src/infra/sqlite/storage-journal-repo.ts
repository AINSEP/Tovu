...
import type { LedgerReadPort, LedgerRow } from "../../features/storage/timeline";
import type { BootLedgerPort, MigrationRunsRepoPort } from "../../features/storage/boot/reconcile-interrupted-migration";
import type { CreateRestorePointRepoPort } from "../../features/recovery/restore-points";
import * as schema from "./storage-journal-schema";
import type { StorageJournalDb } from "./storage-journal-db";

/**
 * @file ADR-041 §2/§4 — real SQLite adapters over the sidecar `ops/storage-journal.db`, closing
 * the gap Session 2's `features/storage` slice explicitly disclosed: the domain-logic layer
 ...
 * `features/storage`/`features/recovery` — no port shape changes, only real persistence behind
 * them. `SqliteStorageLedgerRepo` doubles as `LedgerReadPort` (Timeline's read side) and
 * `BootLedgerPort` (boot reconciliation's append side), since both operate on the same
 * `storage_ledger` table.
 *
 * How it relates to the project:
 * `server/deps.ts` constructs these against the sidecar db `storage-journal-db.ts` opens
...
export class SqliteStorageLedgerRepo implements LedgerReadPort, BootLedgerPort {
  constructor(private readonly deps: { db: StorageJournalDb; siteId: string }) {}
 ...
 * `CreateRestorePointRepoPort`, plus the storage-side persistence helper `db-ops`'s
...
export class SqliteRestorePointsRepo implements CreateRestorePointRepoPort {
  ...
  /** REQ-05 (recovery) / REQ-22 (storage) — persists one `restore_points` row. */
  async save(row: {
