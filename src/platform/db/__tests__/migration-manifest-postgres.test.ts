/**
 * @file Live-Postgres proofs behind `src/platform/db/migration/manifest.ts`'s semantic classifications.
 *
...
/**
 * Matches ONLY the exact decimal shape `process.pid` can ever produce as a string: one or more
 * digits, no leading zero, anchored at both ends. This is checked BEFORE any numeric coercion — a
 * suffix failing it is never even handed to `Number()`.
 *
 * `Number()` coercion — the ORIGINAL bug this regex fixes (round-3 audit, 2026-08-12) — accepts far
 * more than a canonical decimal pid. Verified live against this exact sweep, each one a valid
 * unquoted Postgres identifier suffix that `process.pid` can never itself produce:
 *
 *   suffix            Number(suffix)   old (`Number.isInteger` + `pid > 0`) guard's verdict
 *   "9e8"             900000000        passed  (scientific notation)
 *   "0x10"            16               passed  (hex)
 *   "900000000.0"     900000000        passed  (trailing ".0")
 *   "+12"             12               passed  (leading sign)
 *
 * So a database named e.g. `tovu_migration_fixture_9e8` — one this suite never generated, since
 * `FIXTURE_DB`/the sweep's own reclaimed names are always `FIXTURE_DB_PREFIX + "_" + String(pid)` —
 * was silently parsed by the old guard as pid 900000000, found dead (`ESRCH`), and DROPPED. See the
 * round-3-audit tests below for the live reproduction of exactly that drop, and of this fix.
 */
const CANONICAL_PID_SUFFIX = /^[1-9]\d*$/;

/**
 * Quotes `identifier` as a Postgres double-quoted identifier (doubling any embedded `"`). Used for
 * the one class of name this suite ever DROPs or CREATEs that it did not itself build from known-safe
 * parts (`FIXTURE_DB_PREFIX` + this run's own `process.pid`, always a plain decimal, always a safe
 * bare identifier already): a name read back off `pg_database` in the stale-fixture sweep below (its
 * origin is "whatever anyone on this server ever created"), and the adversarial fixture names the
 * round-3-audit regression tests below construct on purpose. `pg-fixture.ts`'s own
 * `dropDatabase`/`recreateDatabase` stay unquoted (out of this task's ownership scope, and every other
 * caller in this repo only ever passes one of those known-safe names) — this file names its own
 * quoted variants rather than widening that shared helper.
 */
...
/** Quoted-identifier counterpart to `pg-fixture.ts`'s `recreateDatabase`. Needed only by the
 * round-3-audit adversarial-name tests below, which deliberately construct names (containing `.`,
 * `+`, a leading space, ...) that are not even syntactically valid as an UNQUOTED identifier — the
...
function sweepStaleFixtureDatabases(): void {
  ...
  for (const name of allDatabaseNames) {
    ...
    const suffix = name.slice(FIXTURE_DB_PREFIX.length + 1);
    // Canonical-shape check FIRST, before any numeric coercion — round-3 fix. A suffix that doesn't
    // look exactly like a bare decimal pid was never generated by this suite (see CANONICAL_PID_SUFFIX's
    // own doc for the concrete Number()-coercion bug this closes) and must be left strictly alone,
    // regardless of what Number() would make of it.
    if (!CANONICAL_PID_SUFFIX.test(suffix)) continue;
    const pid = Number(suffix);
    if (!Number.isSafeInteger(pid)) continue; // canonical-shaped but implausibly long digit string — refuse to treat it as a real pid at all
    if (isProcessAlive(pid)) continue; // a live concurrent sibling run's database — must not be touched
    dropDatabaseQuoted(name); // identifier-quoted — defense-in-depth on top of the canonical check above (round-3 fix)
  }
}
...
// --- BLOCKER (round-3 audit, 2026-08-12): the sweep can DROP a database that is not one of ours -----
//
// The bug lived in Number() coercion, not in isProcessAlive or the pid<=0 check: "9e8", "0x10",
// "900000000.0", and "+12" are all valid unquoted Postgres identifier suffixes that process.pid can
// NEVER itself produce, yet Number() happily turns every one of them into a plausible-looking positive
// integer, which then reads as a "dead pid" and gets dropped. Reproduced live below by actually
// creating a database under each such name and proving the sweep now leaves every one of them alone —
// before this fix (CANONICAL_PID_SUFFIX did not exist, sweepStaleFixtureDatabases went straight from
// name.slice(...) into Number(...)), this same test failed: every non-canonical name in the table
// below except the pure-empty and non-numeric cases was silently dropped, because Number() coerced
// each one into an integer the old `Number.isInteger(pid) && pid > 0` guard accepted.

test("sweepStaleFixtureDatabases: a non-canonical suffix Number() would coerce into a plausible dead pid is never touched, even though the pre-fix Number()-based guard would have dropped it", () => {
  // Every one of these is a real Postgres identifier suffix (quoted where the bare unquoted grammar
  // can't express it, e.g. the "." in "900000000.0" or the leading "+"/space) that process.pid, which
  // only ever stringifies as a bare decimal with no leading zero, can never itself produce.
  const adversarialSuffixes = [
    "9e8", // Number("9e8") === 900000000 — scientific notation
    "0x10", // Number("0x10") === 16 — hex
    "900000000.0", // Number("900000000.0") === 900000000 — trailing ".0"
    "+12", // Number("+12") === 12 — leading sign
    " 12", // Number(" 12") === 12 — leading whitespace
    "007", // Number("007") === 7 — leading zero (real pids never render this way)
    "", // Number("") === 0 — empty suffix
    "abc", // Number("abc") === NaN — non-numeric, sanity floor
  ];
  const names = adversarialSuffixes.map((suffix) => `${FIXTURE_DB_PREFIX}_${suffix}`);

  for (const name of names) recreateDatabaseQuoted(name);
  try {
    for (const name of names) assert.ok(databaseExists(name), `setup: "${name}" must exist before sweeping`);

    sweepStaleFixtureDatabases();

    for (const name of names) {
      assert.equal(databaseExists(name), true, `"${name}" must be left completely untouched — its suffix is not a canonical decimal pid`);
    }
  } finally {
    for (const name of names) dropDatabaseQuoted(name);
  }
});

test("CANONICAL_PID_SUFFIX: matches only a bare decimal pid shape; rejects every adversarial suffix Number() would otherwise coerce", () => {
  assert.equal(CANONICAL_PID_SUFFIX.test(String(process.pid)), true, "a real pid's own string form must always match");
  assert.equal(CANONICAL_PID_SUFFIX.test("1"), true);
  assert.equal(CANONICAL_PID_SUFFIX.test("42"), true);
  for (const bad of ["9e8", "0x10", "900000000.0", "+12", " 12", "12 ", "007", "0", "", "abc", "-12", "1.5"]) {
    assert.equal(CANONICAL_PID_SUFFIX.test(bad), false, `"${bad}" must not match — it is not the exact decimal shape process.pid produces`);
  }
});
