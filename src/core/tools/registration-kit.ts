 ...
 * the run's `contextRef` (`server/modules/assistant.ts`), and the daemon hands it back to the
 * handler. So a content-type change made by the assistant and one the same person made by clicking
...
export async function requireToolPermission(
  ...
  required: { principalId: string; permission: string; entityType?: string; entityId?: string },
): Promise<void> {
...
export function assertToolIsWirable(params: {
  ...
  if (catalogEntry.actorClassRule && ACTOR_CLASS_RULES_REQUIRING_CONFIRMATION_TRANSPORT.has(catalogEntry.actorClassRule)) {
    throw new Error(
      `tool-registrations: '${toolId}' declares actorClassRule '${catalogEntry.actorClassRule}', which requires a human-confirmation transport Tovu has not wired — leave it unwired until one exists (see ACTOR_CLASS_RULES_REQUIRING_CONFIRMATION_TRANSPORT)`,
    );
  }
}
