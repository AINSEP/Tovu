/**
 * @file Enforces `src/db/migration/pg-fixture.ts`'s own module doc: "this file is TEST
 ...
 * naming `pg-fixture`, and assert every match lives in this directory (`src/db/__tests__/`), which is
...
test("pg-fixture is imported only from src/db/__tests__/ (it shells out to psql and must never reach product code)", () => {
  const files: string[] = [];
  ...
  assert.deepEqual(
    offenders,
    [],
    `pg-fixture.ts must only be imported from src/db/__tests__/, but found an import in: ${offenders.join(", ")}`
  );
});
