 ...
 *    not-yet-generated `(sha256, transformName, version)` run `sharp`
 *    exactly once (`withRenditionLock`, `transform-lock.ts`), via a
 *    double-checked lookup inside the lock so a waiter that lost the race
 ...
 *  - Eager generation for a hot set via the ADR-009 outbox worker.
 *  - Out-of-process generation (a worker thread/process to protect the host
 *    from `sharp` OOMing, ADR-027 §1/§4). This build calls
 *    `ImageTransformerPort.transform` in-process, synchronously awaited
 *    within the request.
...
import type { AssetBlobRepoPort, AssetRenditionRepoPort, BlobStorePort, MediaRepoPort, TransformDefinitionRepoPort } from "./ports";
import type { AssetRenditionRecord } from "./types";
import { mimeForTransformFormat } from "./transform-types";
import { isLatestTransformVersion, isReferencedByPublishedContent } from "./transform-registry";
import { withRenditionLock } from "./transform-lock";
import type { ImageTransformerPort } from "./image-transformer";
 ...
 * dominated by `imageTransformer.transform`'s cost (`sharp`'s native work,
 * out of this function's control).
 * @overallScore 95
