 ...
 *    now; a SQLite adapter is deferred (see `src/server/deps.ts` file-header note
 *    — the same disclosed precedent the last several admin-section libraries in
 *    this repo followed for their own repo ports).
...
import type { AssetBlobRecord, AssetRenditionRecord, BlobGcJournalEntry, MediaRecord } from "./types";
