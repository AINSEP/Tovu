...
import { resolveMediaRendition } from "../rendition-service";
import { registerTransform } from "../transform-registry";
import { uploadMedia, trashMedia } from "../media-service";
import {
...
import { InMemoryImageTransformer } from "../image-transformer";
...
test("resolveMediaRendition: lookup ignores cosmetic slug/ext — the (assetId, transformName, version) triple is the only key", async () => {
  // The route layer strips slug/ext before calling this service at all (ADR-027 §4: they are
