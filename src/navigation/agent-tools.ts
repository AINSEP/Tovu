 ...
 *
 * Precedent already in this repo: `navigation/contracts.ts` declares `NAVIGATION_AI_TOOLS` — a
 ...
 * layer" HITL gateway; the list itself is unused anywhere in this codebase today). Two of its five
 ...
 * likewise this codebase's "search" for their own domains), and `navigation.where_used` has no
 ...
 * Unlike Forms/Identity/Widgets, `menu-service.ts`'s functions perform NO internal `authorize()`
 * call of their own (confirmed: `CreateMenuDeps`/`UpdateMenuTreeDeps`/`AssignLocationDeps` carry no
 * `authorize` field) — every admin HTTP route in `server/routes/admin/menus/*.ts` checks
 * `deps.authorize(...)` inline, before calling the service function (mirrors `media`'s identical
 * gap, see `media/agent-tools.ts`'s header). `tool-registrations.ts` therefore performs that SAME
 * inline `authorize()` call itself (the identical permission string and `entityType: "menu"` the
 * HTTP route uses) rather than relying on a domain-layer gate that does not exist for this library.
 *
 * Architectural role:
...
/**
 * The Navigation domain's fixed agent-tool catalog — the 5 operations the admin HTTP surface
