...
import { getStorageAgentToolCatalog } from "../../agent-tools";
 ...
 * }
 * export function getStorageAgentToolCatalog(required?: {}, optional?: {}): AgentToolDefinition[];
 * ```
 */

test("AC-24 / REQ-20: every read tool in the catalog requires only 'storage.read'", () => {
  const catalog = getStorageAgentToolCatalog();
  const readTools = catalog.filter((t) => t.sideEffects === "none");

  assert.ok(readTools.length >= 5, "REQ-20 names five free-read tools");
  for (const tool of readTools) {
    assert.equal(tool.authorization.permission, "storage.read");
  }
});
...
test("AC-28 / REQ-23: an agent seeking to trigger a restore only finds a guidance tool (storage_get_restore_guidance), never a lever that executes a restore itself", () => {
  const catalog = getStorageAgentToolCatalog();
