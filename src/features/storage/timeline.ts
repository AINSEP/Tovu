 ...
 * write-chokepoint/authorize/append-only-revision model (ADR-041 "Why Storage, not Database").
 *
 * How it relates to the project:
 * `LedgerReadPort` is implemented by the sidecar ops-journal adapter (ADR-041 §2); this module
 * has no knowledge of that adapter's storage details.
 *
 * Architectural role:
 * `features/storage` domain logic. Depends only on the injected `LedgerReadPort`.
 */
