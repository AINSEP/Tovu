/**
 * @file SPEC-017 C-110 / REQ-20–REQ-23 / AC-24 / AC-25 / AC-28 / AC-33 — the Storage domain's
 ...
 * `storage.read`; the one destructive tool (`storage_execute_migrate_forward`) is token-gated and
 ...
 * lever (ADR-041 §6, "Restore is a Recovery tool, not a Storage tool").
 *
 ...
 *
 * Architectural role:
 * `features/storage` domain logic. No dependencies.
 */
...
/**
 * Returns the Storage domain's agent-tool catalog (ADR-041 §6, D4).
 *
 * @complexity O(1) — a fixed, statically-defined list.
 * @overallScore 100
 */
export function getStorageAgentToolCatalog(
  _required: Record<string, never> = {},
  _optional: Record<string, never> = {}
): AgentToolDefinition[] {
  return [
    { name: "storage_get_health", sideEffects: "none", authorization: { permission: "storage.read" } },
    { name: "storage_get_schema_state", sideEffects: "none", authorization: { permission: "storage.read" } },
    { name: "storage_list_pending_migrations", sideEffects: "none", authorization: { permission: "storage.read" } },
    { name: "storage_query_timeline", sideEffects: "none", authorization: { permission: "storage.read" } },
    { name: "storage_list_restore_points", sideEffects: "none", authorization: { permission: "storage.read" } },
    {
      // storage_plan_migrate_forward is a read (ADR-041 §6): it recomputes and returns a plan,
      // never mutates durable state.
      name: "storage_plan_migrate_forward",
      sideEffects: "none",
      authorization: { permission: "storage.read" },
    },
    {
      name: "storage_execute_migrate_forward",
      sideEffects: "mutates-durable-state",
      authorization: { permission: "storage.migrate" },
