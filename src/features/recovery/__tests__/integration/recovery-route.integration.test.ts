 ...
 * never a tab inside Storage; no raw row-edit/SQL-console affordance lives under Recovery; the
 ...
 * Storage — asserted here as "Recovery is its own top-level admin section, not a child of any
 * Storage-like section"), AC-32 (no raw-DB affordance anywhere in Recovery's own section
 * definition), AC-37/AC-38 (no `/admin/backups` entry remains; only the Tier-3 browser, under
 * Storage, remains of `/admin/database`).
 *
...
test("AC-01/AC-02: a distinct 'recovery' admin section exists, independent of any storage/timeline section (never a child/tab of it)", () => {
...
test("AC-38: no standalone 'database' admin section exists — only the ADR-041 Tier-3 read-only browser (under Storage) may remain, per REQ-22/REQ-27", () => {
  const databaseSection = adminSections.find((s) => s.id === "database" || /^database$/i.test(s.label));

  assert.equal(databaseSection, undefined, "a standalone Database nav entry must not exist; the Tier-3 browser is surfaced under Storage, not as its own top-level section");
});
