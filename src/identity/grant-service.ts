...
import { authorize, resolveEffectivePermissions, type AuthorizeDeps } from "./authorize";
...
export async function createUser(required: {
  deps: AuthServiceDeps;
  input: { workspaceId: UUID; callerPrincipalId: UUID; username: string; email?: string; password: string };
...
export async function createPolicy(required: {
  deps: AuthServiceDeps;
  input: { workspaceId: UUID; callerPrincipalId: UUID; name: string; description?: string };
