...
export async function updateUser(required: {
  deps: AuthServiceDeps;
  input: { workspaceId: UUID; callerPrincipalId: UUID; principalId: UUID; email?: string };
...
export async function updatePolicy(required: {
  deps: AuthServiceDeps;
  input: { workspaceId: UUID; callerPrincipalId: UUID; policyId: UUID; name?: string; description?: string };
