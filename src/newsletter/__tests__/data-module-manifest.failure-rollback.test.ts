 ...
 *
 * `declareDataModule()` (`src/features/plugins/data-module.ts`) is spike-quality code — its own
 * header discloses it was written to "surface real problems," predating ADR-023's ACCEPTED status
 * — now made load-bearing for Newsletter's five real production tables. This is the dedicated
 * integration test forcing a mid-DDL failure against Newsletter's REAL 5-table manifest
...
test("data-module-manifest: a mid-DDL failure against the REAL 5-table manifest rolls back to the exact pre-existing state (never-brick, T010 gate)", async () => {
  ...
  // No orphaned rows / no drift in the live db's table count or pre-existing content.
  const postFailureTableCount = (
    db.prepare(`SELECT COUNT(*) AS n FROM sqlite_master WHERE type = 'table'`).get() as { n: number }
  ).n;
  assert.equal(postFailureTableCount, preExistingTableCount, "table count is byte-for-byte unchanged after rollback");
