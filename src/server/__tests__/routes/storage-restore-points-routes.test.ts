...
/**
 * @file design-spec.md §3.8 backend-gap closure — route-level tests for Storage's restore-points
...
test("storage restore-points routes: create -> list golden path, persists the real captured watermark (REQ-22/AC-26/AC-27)", async (t) => {
  ...
  const listRes = await fetch(`${baseUrl}/api/admin/v1/storage/restore-points`, { headers: { cookie } });
...
test("storage restore-points routes: an empty site lists zero restore points, not an error", async (t) => {
  const { app } = buildTestApp();
  const { baseUrl, cookie } = await bootAuthenticated(app, t);

  const res = await fetch(`${baseUrl}/api/admin/v1/storage/restore-points`, { headers: { cookie } });
