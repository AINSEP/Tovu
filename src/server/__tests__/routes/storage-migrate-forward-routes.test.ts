...
/**
 * @file SPEC-017 C-103/C-105 — real-HTTP integration tests for the storage `migrate-forward`
...
function buildTestApp(): { app: express.Express; deps: RouteDeps } {
  ...
  registerAdminStorageMigrateForwardRoutes(app, deps);
  return { app, deps };
}

test("storage migrate-forward: plan -> confirm -> execute succeeds end-to-end and records a restore point + ledger row", async (t) => {
  const { app, deps } = buildTestApp();
  const { baseUrl, cookie } = await bootAuthenticated(app, t);

  const planRes = await fetch(`${baseUrl}/api/admin/v1/storage/migrate-forward/plan`, { method: "POST", headers: { cookie } });
  ...
  const confirmRes = await fetch(`${baseUrl}/api/admin/v1/storage/migrate-forward/confirm`, {
    method: "POST",
  ...
  const executeRes = await fetch(`${baseUrl}/api/admin/v1/storage/migrate-forward/execute`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
...
test("storage migrate-forward: a stale plan hash at confirm time is rejected before a token is minted", async (t) => {
...
test("storage migrate-forward: replaying an already-redeemed confirmation token is rejected TOKEN_ALREADY_REDEEMED", async (t) => {
  const { app } = buildTestApp();
  const { baseUrl, cookie } = await bootAuthenticated(app, t);
  ...
  const firstExecuteRes = await fetch(`${baseUrl}/api/admin/v1/storage/migrate-forward/execute`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({ confirmationToken }),
  });
  ...
  const replayRes = await fetch(`${baseUrl}/api/admin/v1/storage/migrate-forward/execute`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({ confirmationToken }),
  });
...
test("AUD-001 regression: an unauthorized (bare) principal is rejected at /execute before any lock acquisition, with any confirmationToken", async (t) => {
  ...
  assert.equal(body.details.permission, "storage.migrate");
  ...
  assert.equal((await deps.storageLedgerRepo.query({ limit: 10 })).items.length, 0);
});
