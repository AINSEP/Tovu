...
import { createSqliteRouteDeps, defaultStorageJournalDbPath } from "../../deps";
...
import { SqliteMigrationRunsRepo } from "../../../infra/sqlite/storage-journal-repo";
 ...
 * `runBootLifecycle()` path `index.ts` uses, against a real sidecar `ops/storage-journal.db` file.
 *
...
test("a non-terminal migration_runs row (simulated crash mid-migration) is reconciled at boot: siteStatus flips to BLOCKED_PENDING_RECOVERY and a migration.interrupted ledger row is appended", async () => {
  ...
  try {
    ...
    fs.mkdirSync(path.dirname(storageJournalDbPath), { recursive: true });
    const storageJournalDb = openStorageJournalDb(storageJournalDbPath);
    const migrationRunsRepo = new SqliteMigrationRunsRepo({ db: storageJournalDb, siteId: "workspace-local" });
    ...
    (storageJournalDb as unknown as { $client: { close(): void } }).$client.close();
    ...
    const reconciliation = result.modules.find((m) => m.name === "storage-migration-reconciliation");
    ...
    const ledgerPage = await deps.storageLedgerRepo.query({ kind: "migration.interrupted", limit: 10 });
...
test("ADR-041/043/044/045 re-audit round 2 (codex finding R2-F2-BLOCK-NOT-ENFORCED): a real HTTP request to a normal admin route or the public site is actually refused (503 SITE_BLOCKED_PENDING_RECOVERY) while siteStatus is BLOCKED_PENDING_RECOVERY -- detection alone (siteStatus flipping) is not enough; Recovery, auth, and healthz stay reachable so an operator can resolve it", async (t) => {
  ...
  try {
    ...
    const normalAdminRoute = await fetch(`${baseUrl}/api/admin/v1/storage/timeline`, { headers: { cookie } });
...
test("round-5 re-audit (codex R5-F1-BLOCKED-RECOVERY-NOT-RESTART-SAFE / Fable R5-F1-INTERRUPTED-SECOND-BOOT-BRICK): a SECOND boot against the same sidecar, with the same still-unresolved migration, re-detects and re-blocks WITHOUT crashing the critical boot module", async () => {
  ...
  try {
    ...
    const reconciliation1 = result1.modules.find((m) => m.name === "storage-migration-reconciliation");
    ...
    const reconciliation2 = result2.modules.find((m) => m.name === "storage-migration-reconciliation");
    ...
    const ledgerPage = await deps2.storageLedgerRepo.query({ kind: "migration.interrupted", limit: 10 });
...
test("round-5/6 re-audit (codex R5-F1 / Fable R5-F2-BLOCK-HAS-NO-EXIT, then codex round-6 R6-F1-RESTART-REQUIRED-RESTORE-UNBLOCKS-STALE-DB): a successful restore durably resolves the migration, but the running process stays blocked until a real restart -- clearing it in-process would serve stale pre-restore data from the old, now-unlinked file handle", async (t) => {
  ...
  try {
    ...
    const normalAdminRouteBeforeRestart = await fetch(`${baseUrl}/api/admin/v1/storage/timeline`, { headers: { cookie } });
    ...
    const normalAdminRouteAfterReboot = await fetch(`${rebootBaseUrl}/api/admin/v1/storage/timeline`, { headers: { cookie: rebootCookie } });
