 ...
 * real `themes/handlebars/ledger` demonstrator, exercising the full path:
...
function ledgerTheme() {
  const theme = loadTheme({
    themeDir: path.join(process.cwd(), "themes", "handlebars", "ledger"),
    id: "ledger",
    source: "built-in",
  });
...
test("both logic tiers see the identical render-data contract — the same fields resolve in Liquid and in Handlebars", async () => {
  ...
  const liquid = loadTheme({ themeDir: path.join(process.cwd(), "themes", "liquidjs", "dispatch"), id: "dispatch", source: "built-in" });
