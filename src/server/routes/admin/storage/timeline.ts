 ...
 * read-first Timeline). Gated by `storage.read` (ADR-041 §6's permission catalog).
 *
 * The first (and, for this dispatch, only) Storage/Recovery admin route wired end-to-end against
 * a real adapter (`SqliteStorageLedgerRepo`, `server/deps.ts`) — demonstrates the sidecar journal
 * is genuinely reachable over HTTP, following the exact pattern
 * `routes/admin/redirects/list.ts` already established. The remaining Storage/Recovery API
