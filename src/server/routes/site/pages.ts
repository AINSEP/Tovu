...
export const registerSiteRoutes: RouteRegistrar = (app, deps) => {
  app.get("/", async (_req, res) => {
    try {
      ...
      res.type("html").send(await renderSite({ theme, route: "home", siteTitle: SITE_TITLE, posts }));
    } catch {
  ...
  app.get("/:slug", async (req, res, next) => {
    ...
    try {
      ...
      res.type("html").send(await renderSite({ theme, route: "post", siteTitle: SITE_TITLE, posts, post }));
    } catch (err) {
