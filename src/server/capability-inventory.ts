...
export const CAPABILITY_INVENTORY: readonly CapabilityInventoryEntry[] = [
  ...
  },
  {
    ...
    ownerModule: "members",
    classification: "production",
    sourceOfTruth: "in-memory (InMemoryMemberRepo/InMemoryMemberSessionRepo/InMemoryMagicLinkTokenRepo)",
    readinessDependencies: ["durable member/session adapter (Phase 1)"],
    startupCriticality: "critical",
    securityDependencies: ["magic-link rate limiters", "session token store"],
    restartTestOwner: "members test suite",
    hasDurableAdapter: false,
  },
  {
    name: "webhooks",
    ...
    classification: "production",
    sourceOfTruth: "in-memory (InMemoryWebhookSubscriptionRepo/InMemoryWebhookDeliveryRepo); processDueDeliveries worker not yet started anywhere",
    readinessDependencies: ["durable webhook subscription/delivery adapter (Phase 1)", "delivery worker activation (Phase 4)"],
    startupCriticality: "optional",
    securityDependencies: ["KeyringPort-backed webhook signer"],
    restartTestOwner: "integrations test suite",
    hasDurableAdapter: false,
  ...
  },
  {
    ...
    ownerModule: "origin",
    classification: "production",
    sourceOfTruth: "in-memory (InMemoryOriginSettingRepo, seeded with a hardcoded dev-capability localhost origin)",
    readinessDependencies: ["durable origin-settings adapter (Phase 1)", "a real (non-hardcoded) verified production origin"],
    startupCriticality: "critical",
    securityDependencies: ["egress allowlist enforcement"],
    restartTestOwner: "origin test suite",
    hasDurableAdapter: false,
  },
  {
    name: "media",
    ownerModule: "media",
    classification: "production",
    sourceOfTruth: "in-memory rows (InMemoryMediaRepo/InMemoryAssetBlobRepo/InMemoryAssetRenditionRepo/InMemoryTransformDefinitionRepo); blob bytes ARE durable via LocalFsBlobStore",
    readinessDependencies: ["durable media metadata/registry adapter (Phase 1)", "sharp native binary readiness (REQ-08)"],
    startupCriticality: "optional",
    securityDependencies: ["identity authorize() gate (ADR-027 §7 permission set)"],
    restartTestOwner: "media test suite",
    hasDurableAdapter: false,
  },
  {
    name: "analytics",
    ...
    classification: "production",
    sourceOfTruth: "in-memory (LocalBufferSink; self-reports capabilities().durable=true, which INV-03 forbids trusting)",
    readinessDependencies: ["durable analytics sink adapter (Phase 1)"],
    startupCriticality: "optional",
    securityDependencies: [],
    restartTestOwner: "analytics test suite",
    hasDurableAdapter: false,
  },
  {
    name: "store",
