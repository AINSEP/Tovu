...
import type { StorageRecoveryRouteDeps } from "../routes/admin/storage-recovery/deps";
import type { ServerModuleHandle } from "./types";

/**
 * @file ADR-046 Phase 3 (SPEC-042, final slice) — the `storage-recovery` server module (ADR-041
 * Storage Timeline + ADR-045 Recovery).
 *
 ...
 * reordering of gates/middleware — but see `routes/admin/storage-recovery/deps.ts`'s own header
 ...
 *
 * Owns 7 registrations, discovered by reading all 6 source files directly (`storage/timeline.ts`,
 * `storage/restore-points.ts` [2 registrars], `recovery/restore-points.ts`, `recovery/
 * disclosure.ts`, `recovery/deep-link.ts`, `recovery/status.ts`):
 *  - `registerAdminStorageTimelineRoute` (Storage's read-first Timeline)
 *  - `registerAdminStorageRestorePointsListRoute` / `registerAdminStorageRestorePointsCreateRoute`
 ...
 *
 * Deliberately NOT moved: `registerAdminStorageMigrateForwardRoutes` / `registerAdminRecoveryRestoreRoutes`
 ...
 *
 * Ordering note (disclosed): in `app.ts`, the storage 3 registrars and the recovery 4 registrars
 ...
 * the position the storage block used to occupy. This is safe: none of these 7 routes' fixed
 * paths (`/api/admin/v1/storage/...`, `/api/admin/v1/recovery/...`) ever overlap any path
 ...
 */
export function createStorageRecoveryModule(deps: StorageRecoveryRouteDeps): ServerModuleHandle {
  return {
    name: "storage-recovery",
    registerRoutes: (app) => {
      registerAdminStorageTimelineRoute(app, deps);
      registerAdminStorageRestorePointsListRoute(app, deps);
