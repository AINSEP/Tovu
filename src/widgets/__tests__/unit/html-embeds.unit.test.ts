...
/**
 * @file SPEC-047 Slice 2 — `html-embeds.ts`'s `data-widget-embed`/`data-form-embed` scan/substitute
 * pair, tested as the pure string functions they are (no repo, no resolver — see
...
test("scanHtmlEmbeds: finds a widget embed and a form embed, in document order", () => {
  const refs = scanHtmlEmbeds(
    '<p>intro</p><div data-widget-embed="w1"></div><div data-form-embed="f1"></div>'
  );
...
test("scanHtmlEmbeds: the target attribute may appear anywhere among the div's other attributes", () => {
  const refs = scanHtmlEmbeds('<div class="slot" data-widget-embed="w1" data-x="y"></div>');
  assert.deepEqual(refs, [{ kind: "widget", id: "w1" }]);
});

test("scanHtmlEmbeds: whitespace between the opening and closing tag is tolerated", () => {
  const refs = scanHtmlEmbeds('<div data-widget-embed="w1">\n  \n</div>');
  assert.deepEqual(refs, [{ kind: "widget", id: "w1" }]);
});
...
test("scanHtmlEmbeds: a div with real content between the tags does not match — it is not an empty placeholder, so it is left alone rather than misread", () => {
  const refs = scanHtmlEmbeds('<div data-widget-embed="w1"><span>not empty</span></div>');
  assert.deepEqual(refs, []);
});

test("scanHtmlEmbeds: an id longer than the sanity bound is dropped rather than carried into a resolver/entry_refs lookup", () => {
  const longId = "x".repeat(500);
  const refs = scanHtmlEmbeds(`<div data-widget-embed="${longId}"></div>`);
  assert.deepEqual(refs, []);
});

test("scanHtmlEmbeds: an empty id attribute is dropped", () => {
  const refs = scanHtmlEmbeds('<div data-widget-embed=""></div>');
  assert.deepEqual(refs, []);
});

test("scanHtmlEmbeds: truncates at MAX_HTML_EMBEDS_PER_PAGE, never returns more", () => {
  const html = Array.from({ length: MAX_HTML_EMBEDS_PER_PAGE + 20 }, (_, i) => `<div data-widget-embed="w${i}"></div>`).join("");
...
test("substituteHtmlEmbeds: replaces each placeholder with resolve()'s return value, leaves surrounding markup untouched", () => {
  const html = '<p>before</p><div data-widget-embed="w1"></div><p>after</p>';
  const out = substituteHtmlEmbeds(html, (ref) => `[${ref.kind}:${ref.id}]`);
...
test("substituteHtmlEmbeds: a non-matching (non-empty) div is left byte-for-byte unchanged, resolve() is never called for it", () => {
  const html = '<div data-widget-embed="w1"><em>keep me</em></div>';
  let calls = 0;
...
test("substituteHtmlEmbeds: the captured id is handed to resolve() as plain data, never re-interpolated into the output by this function itself — a hostile id cannot break out of resolve()'s own returned markup", () => {
  const html = '<div data-widget-embed="\"><script>alert(1)</script>"></div>';
...
test("substituteHtmlEmbeds: multiple embeds of the same id each resolve independently (resolve() is called once per occurrence, not once per distinct id)", () => {
  const html = '<div data-widget-embed="w1"></div><div data-widget-embed="w1"></div>';
  let calls = 0;
  const out = substituteHtmlEmbeds(html, () => {
    calls += 1;
    return `[${calls}]`;
