 ...
 * Nothing outside `src/http/` itself or the named composition-root files may import it directly
 ...
 */
const ALLOWED_IMPORTERS = new Set(["src/http/client.ts"]);

test("transport.fetch is not imported outside src/http or the composition root", () => {
  const grepOutput = execFileSync(
    "grep",
    ["-rl", "--include=*.ts", "transport.fetch", join(repoRoot, "src")],
    { encoding: "utf8" }
  ).trim();
