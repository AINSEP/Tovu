...
| spec_root | ADS-project-knowledge/specs/021-media-assets |
...
| `ADS-project-knowledge/reports/architecture/ADR-INDEX.md` (Media row, line 36) | index entry | Cross-checked against the real code; the ADR-INDEX summary accurately describes ADR-027's DESIGN (not the build), consistent with this spec's own separation of design-vs-build. |

---

## Validation Notes

- Validator run: `python3 AI-Dev-Shop/framework/spec-providers/speckit/validators/validate_spec_package.py ADS-project-knowledge/specs/021-media-assets --phase spec --update-hash`.
