...
| `.site-meta.json` | file (JSON) | init (last step) | serve (`schemaVersion` bump only) | Commit marker + provenance |
...
SiteMetaJson:                      # .site-meta.json — provenance + compatibility stamp
  ...
  schemaVersion: integer           # migration stamp; monotonically non-decreasing (INV-05)
...
| `SERVE_SITE` | dir, port? | §1 layout valid; `schemaVersion` ≤ runtime | forward-migrates db if older; bumps `schemaVersion` after migration; starts listener | validation miss ⇒ exit 3/4/5 with nothing written except completed idempotent migrations (EC-09) |
...
Illegal states (must be impossible or rejected): `.site-meta.json` present without `content.db`; `schemaVersion` decreasing; serving a dir without the commit marker.
...
| `runtimeSchemaVersion` | — | integer = count of Drizzle migrations bundled under `drizzle/` (from `drizzle/meta/_journal.json`), exported beside the migrator wiring | — |
...
- **Schema version source of truth:** the count of Drizzle migrations the runtime bundles under `drizzle/` (read from `drizzle/meta/_journal.json`), exposed as `runtimeSchemaVersion` beside the migrator wiring. Every `drizzle-kit generate` adds one migration and increments this number — SPEC-002's `kind` migration is one such increment. This install-dir stamp is distinct from Drizzle's per-db `__drizzle_migrations` journal (which records *which* migrations a given `content.db` has applied); the stamp answers the portability question "is this site newer than the runtime?" (REQ-05).
