# External Audit Packet — Round 4 (Compliance Inspector pass)

## Ask

- **User request:** Run round 4 of `/audit-work` on ADR-043 (Collections) and ADR-044 (Categories & Tags) — a confirming compliance pass on the fixes applied after round 3's external audit, before treating these as spec-ready. (ADR-045 is not included: both external auditors found it clean in round 3, no further changes were made to it.)
- **Audit focus:** Do the round-3 fold edits actually satisfy what round-3's auditors (Fable internal, Codex, agy) asked for? Did any edit introduce a new problem or fail to fully resolve the finding it targeted?
- **Scope:** custom (diff-only compliance pass over two design documents, no code)
- **Suggested changes mode:** patches (bounded text-level edits, same as prior rounds)
- **Audit target:** the unified diff described below, plus full current text of both files for context
- **Planned auditors:** codex, agy (gemini) — external peers, same as round 3. No internal-verification pass this round — Fable's round-3 findings ARE most of what's being reconciled below, so re-running Fable as verifier first would be circular; Codex and agy dispatch directly.
- **Authoring packet:** `ADS-project-knowledge/reports/external-audit/packets/20260714-adr-043-044-round4-audit-packet.md`
- **Dispatch packet:** same as authoring packet — self-contained stdin payload

## Threat Model & Scope Contract (frozen, unchanged from round 3)

- **Threat model id:** `TM-ADR-CONTENT-ADMIN-005` — same as rounds 2/3, not a new series (no material threat-model change)
- **Audit round:** 4
- **Intended use / deployment context:** unchanged — pre-implementation ADR design text for Tovu, a self-hosted per-tenant CMS
- **Allowed actors & capabilities:** unchanged — workspace/site admins via admin UI (ADR-021), delegated agents, seeded system principal; no raw DDL or chokepoint bypass by any actor
- **In-scope blocking failure domains (ALLOWLIST):** unchanged, same 5 domains as round 3 (data loss/corruption; authz/tenant-scope violation; destructive-operation-labeled-safe; cross-ADR/dependency inconsistency; undisclosed premature abstraction)
- **Mandatory invariants:** unchanged, same 5 as round 3 (no raw DDL; same-transaction chokepoint discipline; write-time cross-workspace rejection; reversibility claims must be genuine; no uncomputable disclosure claims)
- **Blocking impact threshold:** unchanged — concrete mechanism under normal/documented use, not stylistic preference
- **Risk tier & score floor:** unchanged — medium/high, floor 8.5
- **Gate formula:** unchanged — `blocking_gate = FAIL` iff `(validated unresolved blockers > 0) OR (score < 8.5)`
- **Explicit non-goals:** unchanged, plus: this round does NOT re-litigate anything already resolved as clean by both round-3 external auditors on ADR-045 (out of scope this round entirely) or anything in ADR-043/044 that round 3 did not flag.

## Prior-Round Disposition Ledger

Every finding from round 3 (internal Fable + external Codex + external agy), with disposition and where it was fixed. Match findings by underlying causal claim, not by ID — Fable/Codex/agy used different IDs for the same defects.

| ID | Finding | Disposition | Evidence / rationale |
|---|---|---|---|
| ADR-045-disclosure (Fable F1) | ADR-045 disclosure claimed to count `entries`, uncomputable | `fixed` | Fixed pre-round-3-dispatch; both external auditors confirmed clean in round 3. Not re-audited this round (ADR-045 out of scope). |
| ADR-043/044-watermark (Fable F2) | New chokepoints didn't stamp `storage_write_watermark` | `fixed` | ADR-043 §4 (pre-round-3); ADR-044 Wiring section, new bullet (this fold, round 3) |
| ADR-043-name-grammar (Fable F6) | No key/field-name grammar, DDL injection risk | `fixed` | ADR-043 §4, "Key/field-name grammar" bullet (pre-round-3) |
| ADR-043-kind-grammar (Codex F1, round 3) | Grammar fix covered names, not field `kind`, same DDL-interpolation class | `fixed` | ADR-043 §4, "Round-3 correction" paragraph appended to the grammar bullet |
| ADR-044-allowlist-contradiction (Fable F3, Codex F3, agy F-044-1) | "Until ADR-043 ships" claim impossible given ADR-043's permanent key reservation | `fixed` | ADR-044 §4, Wiring section bullet, AND Consequences section (three occurrences, all reworded to "permanently") |
| ADR-044-lens-injectivity (Fable F4) | Workspace-match validation alone leaves a type/lens mismatch gap | `fixed` | ADR-044 Wiring section, "Workspace-scoped join resolution" bullet, "Round-3 correction" appended |
| ADR-044-merge-reversibility (Fable F5, Codex F2, agy F-044-2) | `mergeTerm` "reversible narrative" claim false given `entry_terms` non-revisioning exemption | `fixed` | ADR-044 Failure modes, "Destructive term merge" bullet, rewritten; write-service.ts interface comment updated |
| ADR-044-sample-completeness (Fable F7, Codex F4, agy F-044-3) | Missing `taxonomy_revisions` sample, outbox obligation, attribution | `fixed` | ADR-044 sample: added `taxonomyRevisions` table; Wiring section: added outbox+watermark bullet |
| ADR-043-vacuous-index (Fable F8, agy F-043-1) | `content_type_revisions` unique index vacuous | `fixed` | ADR-043 §5 sample: added `perContentTypeSeq` |
| ADR-043/044/045-status-lines (Fable F9) | Stale "not audited" claims contradicting inline audit-fold sections | `fixed` | All three status lines rewritten to reflect actual multi-round audit history |
| ADR-044-kind-mutation (agy F-044-4) | Claimed post↔page kind mutation orphans `entry_terms`, needs cascade | `wontfix` | **Verified false against `src/features/post/post.ts`**: `kind` is documented "fixed at creation; v1 has no post<->page conversion path," and `UpdatePostInput` has no `kind` field. No cascade mechanism exists to audit because the premised mutation cannot occur in v1. Noted inline in ADR-044's "Round-3 correction" paragraph as a disclosed non-fix. |

## What changed since round 3 (the diff to audit)

**ADR-043-collections.md:**
1. §4 "Key/field-name grammar" bullet — appended a "Round-3 correction" paragraph adding a closed field-`kind` enum requirement + core-owned SQL-literal mapping (addresses Codex F1).
2. §5 sample — `content_type_revisions` gained a `perContentTypeSeq` column; the unique index now targets `(contentTypeId, perContentTypeSeq)` instead of the vacuous `(contentTypeId, seq)` (addresses Fable F8 / agy F-043-1).
3. Status line — rewritten to describe all three audit rounds accurately (addresses Fable F9).

**ADR-044-categories-and-tags.md:**
4. §4 "Opt-in per content type" — reworded to state the `post`/`page` allow-list is permanent, not "until ADR-043 ships" (addresses Fable F3 / Codex F3 / agy F-044-1, occurrence 1 of 3).
5. Wiring section — reworded the `content_types` registry bullet (occurrence 2 of 3, same finding).
6. Wiring section — new bullet: same-transaction outbox + `storage_write_watermark` stamping obligation for every taxonomy write (addresses Fable F2 (ADR-044 half) / Codex F4 / agy F-044-3, ecosystem-obligations half).
7. Wiring section, "Workspace-scoped join resolution" bullet — appended a "Round-3 correction" paragraph requiring type/lens match, not just workspace match (addresses Fable F4); also explicitly notes agy's kind-mutation cascade concern does not apply, with the `post.ts` citation (disposes agy F-044-4).
8. Concrete sample — added a new `taxonomyRevisions` schema table with `pluginId` attribution (addresses Fable F7 / Codex F4 / agy F-044-3, sample-completeness half).
9. `write-service.ts` sample interface — one-line comment update on `mergeTerm` flagging the revised reversibility disclosure.
10. Failure modes, "Destructive term merge" bullet — rewritten to disclose merge as non-reversible for membership rows, removing the false "(reversible narrative)" claim (addresses Fable F5 / Codex F2 / agy F-044-2).
11. Consequences section — reworded the third occurrence of the "until ADR-043 ships" claim (occurrence 3 of 3, same finding as #4/#5 — this occurrence was MISSED in the original round-3 fold and caught by the Coordinator's own re-check before this round-4 dispatch; flagging this explicitly since a round-4 auditor might otherwise wonder why a "fixed" finding still had a stale trace).
12. Status line — rewritten to describe all three audit rounds accurately, including the disposed agy F-044-4 finding.

## Full current text — ADR-043 (post-round-3-fold)
...
- Status: **PROPOSED** — emerged from a 4-round swarm `/debate` (2026-07-14, Primary Sonnet host +
  ...
  **not** converge through Rounds 1-3 — each of the three participants held a different final position
  ...
  Round 4 (Fable 0.85, Codex 0.86, agy 0.95). **Audited three times under two threat models: round 1 under
  `TM-ADR-STORAGE-CONTENT-004` (2026-07-14, Coordinator-overridden FAIL, 4 fixes folded — see §6 below); an
  internal-verification round under `TM-ADR-CONTENT-ADMIN-005` (2026-07-14, cross-ADR hard-blocker root
  cause + a DDL-grammar gap, both fixed); and external peer re-dispatch (2026-07-14, Codex GPT-5.6 Terra +
  Gemini 3.1 Pro, same TM id) which found one further blocker (the grammar fix covered field names but not
  field *kind*, also interpolated into cast DDL — fixed this fold, see §4) plus a low-severity vacuous-index
  finding (fixed, see §5). No further blockers found by either external auditor on this ADR — see the
  external-audit run for full cross-auditor reasoning. Still requires human owner sign-off before Accepted.**
- Date: 2026-07-14
...
- **Key/field-name grammar (added, round-2 `/audit-work` fold, TM-ADR-CONTENT-ADMIN-005; extended round 3).**
  ...
  **Round-3 correction (2026-07-14, external audit — Codex): the round-2 fold constrained key/field* names*
  ...
  path is core-assigned and grammar-validated the same way before index construction.**
...
## Full current text — ADR-044 (post-round-3-fold)
...
- Status: **PROPOSED** — emerged from the same 3-round swarm `/debate` as ADR-043 (2026-07-14). Unlike
  ...
  three rounds** — there is no persistent disagreement to tie-break here. **Audited three times under two
  threat models: round 1 under `TM-ADR-STORAGE-CONTENT-004` (2026-07-14, unanimous FAIL — the only one of
  ...
  fixed at creation with no post↔page conversion path in v1. See the external-audit run for full
  cross-auditor reasoning and disposition. Still requires human owner sign-off before Accepted.**
- Date: 2026-07-14
...
- **ADR-009 outbox discipline + ADR-041 §5 watermark stamping (added, round-3 `/audit-work` fold,
  ...
  write (`assignTerms`, `mergeTerm`, `renameTerm`, and `taxonomies`/`terms` create/deprecate) must enqueue an
...
export const taxonomyRevisions = sqliteTable("taxonomy_revisions", {
  ...
  op: text("op").notNull(),                     // create|rename|merge|deprecate
  recordedAt: text("recorded_at").notNull(),
}, (t) => [index("idx_taxonomy_revisions_taxonomy_seq").on(t.taxonomyId, t.seq)]);
...
This is **round 4** — act as a Compliance Inspector, not a fresh full-pass auditor. Your task:

1. **Reconcile the Prior-Round Disposition Ledger above.** For each `fixed` entry, verify the cited fix actually resolves the finding as stated — do not assume it does because it's marked fixed. For the one `wontfix` entry (agy's kind-mutation finding), independently assess whether the Coordinator's rejection rationale holds (the cited code claim — `posts.kind` fixed at creation, no conversion path, `UpdatePostInput` has no `kind` field — is asserted, not shown; you do not have direct repo access this round, so evaluate it as a claim: is it internally consistent with everything else in these ADRs? Flag if suspicious, don't just accept it because the Coordinator asserted it).
2. **Audit the numbered diff (`What changed since round 3`) plus the minimum surrounding context needed to judge behavioral effects** — do not re-audit untouched sections of either ADR for new issue classes. A finding not causally connected to one of the 12 numbered changes above, or to reconciling the ledger, needs a `round_1_miss_justification` citing genuinely new evidence, per the standard round-N late-finding rule.
3. Every round-N finding must state its `diff_causal_link` — which of the 12 numbered changes (or which ledger entry) it's about.
4. Same threat-model handshake, blocker rule, and escape valve as round 1 (frozen contract above, unchanged).
...
```json
{
  ...
  "findings": [{
    ...
    "diff_causal_link": "<which of the 12 numbered changes, or which ledger entry, this concerns>",
    "round_1_miss_justification": "<only if this is a genuinely new finding not tied to the diff — cite new evidence>",
  ...
  "closure": "COVERAGE_COMPLETE — round-4 compliance coverage finished under TM-ADR-CONTENT-ADMIN-005; see blocking_gate for the pass/fail outcome."
}
```

Then prose: what looks solid; `Suggested Changes` + `Proposed File Changes` (patches mode) for anything you'd still change, bounded to files you actually reviewed.
