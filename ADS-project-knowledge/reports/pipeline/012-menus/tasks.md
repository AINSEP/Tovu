...
- Outline: `ADS-project-knowledge/reports/pipeline/012-menus/implementation-outline.md` (Status: PRODUCED)
...
- [ ] T002 **[GATE]** Confirm `/audit-work` has run against this ADR (recommended in the ADR's own acceptance note, given the shared permission-migration mechanism is security-adjacent and reused by up to three sibling remediations), **or** an explicit human-approved waiver deferring that audit is recorded in `pipeline-state.md`. **This gate does NOT block Phase 0/1 test-writing or the mechanism's own implementation and testing** — it specifically blocks **T014** (wiring `migrateDeprecatedPermissionGrants()` into the live `seedIdentity()` boot path) only. The Programmer may build and fully test `permission-migrations.ts` now; T014 itself must wait for a real `/audit-work` pass, not a Coordinator waiver — this one is not being waived, given the security-adjacent, multi-feature-reused nature of the mechanism.
...
- [ ] T013 **[depends on T002 GATE]** Wire `migrateDeprecatedPermissionGrants()` into `seedIdentity()`'s existing boot path, called once, immediately after built-in roles/policies are seeded — mirrors `migrateLegacyPresentationSettings()`'s call-site pattern (ADR-PIPE-007 precedent) — `src/identity/seed.ts` (depends T007, T011, **and T002's `/audit-work`-or-waiver gate — do not start this task until that gate is satisfied**)
...
- [ ] T047 [P] Update `ADS-project-knowledge/specs/012-menus/traceability.spec.md` REQ-14/AC-19/AC-20 rows from "gated by `navigation.manage`" to the new per-action checks — or, if the Coordinator elected Article II remediation path (a) (T001), fold into the SPEC-012 v1.1 amendment instead of editing the as-built rows directly
