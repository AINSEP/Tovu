...
**Phase 3 status (2026-07-16, later same day): second slice complete (SPEC-034); still NOT fully migrated — disclosed, intentional.** Pulled the next 3 self-contained domains off the "still owed" list above: `media.ts` (5 admin routes + the public rendition route, all genuinely narrow-typed against a new `MediaRouteDeps` Pick, not the full `RouteDeps` locator), `taxonomy.ts` (the 5 plain CRUD/list routes, narrow-typed against a new `TaxonomyRouteDeps`), and `integrations-admin.ts` (the 5 webhook-subscription admin CRUD/read routes — distinct from SPEC-031's `integrations.ts`, which owns the fan-out subscriber only; `routes/admin/integrations/deps.ts`'s `IntegrationsRouteDeps` was itself narrowed from a historical `extends RouteDeps` widening to a real `Pick`, now that `RouteDeps` carries the fields directly). `ServerModuleHandle` now has 6 real consumers. The route-class precedence bullet is now enforced structurally, not just by comment: `route-class-precedence.unit.test.ts` introspects the real `createApp()` Express router stack and asserts `GET /:slug` registers strictly last. Verified as a pure refactor: full suite (1660 tests) unchanged except the 2 new structural-test cases, same 4 pre-existing unrelated failures, zero new ones; typecheck clean. **`requireAdminSession` relocation was attempted, not merely deferred again** — investigation found it genuinely order-entangled with `registerAuthRoutes` (`POST /api/admin/v1/auth/login` must register before the `/api/admin` session gate mounts, or login itself becomes ungettable), which `core.ts`'s current zero-argument, SPEC-031-established shape doesn't accommodate without either absorbing `registerAuthRoutes` too (a different concern) or widening `ServerModuleHandle` past its single-`registerRoutes`-call shape; left inline, with the exact blocking mechanics recorded in SPEC-034 for whoever pulls it next. **Still owed, explicitly**: the remaining ~45 `app.ts` route registrations (posts, pages, change-sets, presentation, members, menus, users/roles/policies, settings, forms admin, redirects, storage/recovery, content-types/entries, SEO, comments moderation, analytics), `requireAdminSession`+`registerAuthRoutes` as a paired relocation, any `deps.ts` split, and a `content.ts` module. SPEC-034's own Non-Goals section is the authoritative list.
