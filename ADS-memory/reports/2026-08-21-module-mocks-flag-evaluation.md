...
- `development/scripts/rerun-failing-tests.sh` still lacks the flag (out of this task's scope; noted above).
- The two files whose comments originally motivated this (`admin-assistant-execution-routes.test.ts`, `database-migrate-forward-routes.test.ts`) were **not rewritten** to use `mock.module()` — that was not requested, and given the `app.js` eager-load trap, doing it properly means bypassing `createRouteDeps()` for those specific tests the way the new file does, which is a real (if mechanical) rewrite, not a drop-in.
