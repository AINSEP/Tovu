...
Both of the team lead's hypothesized explanations for layer 1's mystery (an unredirected child spawn, or an indirect `firstExportFailureLazily`-style reach-point) are now ruled out for the 12-file `http` cluster specifically. **Two independent people have now read the code for this cluster looking for a call-site cause and found nothing — this session's predecessor on the worker-thread angle, this round on the spawn/reach-point angle.** That is not merely an absence of evidence; a negative result from two independent searches, each following a different plausible mechanism to its actual code, is itself a finding. Combined with layer 2's confirmed non-determinism (below), the weight of evidence now favors "a property of the coverage-collection/merge process itself" over "an undiscovered call site" as the working explanation for layer 1.
