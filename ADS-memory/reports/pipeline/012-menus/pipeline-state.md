...
| provider_output_root | ADS-memory/specs/012-menus |
| spec_path | ADS-memory/specs/012-menus |
| spec_entrypoint_path | ADS-memory/specs/012-menus/feature.spec.md |
| spec_readiness_artifact | ADS-memory/specs/012-menus/spec-dod.md |
...
| pipeline_adr | ADR-PIPE-012 (Menus Remediation — Permission Catalog Rename/Split, SQLite Persistence, Outbox Events, Binding-Index Reconciliation), PROPOSED 2026-07-13 — `ADS-memory/reports/pipeline/012-menus/adr.md` |
...
| implementation_outline_path | `ADS-memory/reports/pipeline/012-menus/implementation-outline.md` — PRODUCED (triggers: Boundary Cross, Contract Change, Data And Persistence, Brownfield Dependency, Critical Cross-Boundary Invariant, Parallelization Ambiguity) |
...
- **Scope of this dispatch:** Spec Agent, lightweight as-built backfill only (not the full
  ...
  `spec-manifest.md`, `spec-dod.md`) at `ADS-memory/specs/012-menus/`.
...
- **Validator not yet run in this message.** `content_hash` fields in every `.spec.md` file are
  ...
  `python3 AI-Dev-Shop/framework/spec-providers/speckit/validators/validate_spec_package.py ADS-memory/specs/012-menus --phase spec --update-hash`
...
- **Scope:** remediation ADR + implementation outline only (per dispatch instruction) — no
  ...
  `ADS-memory/specs/012-menus/` (`spec-manifest.md` first) → `constitution.md` →
