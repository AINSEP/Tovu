...
- **Sequencing.** Decision 1 (forms) ships first, but is **not** independent — see the amendment below. It requires Decision 5 and Decision 7. Decisions 2–4 and 6 follow.
  - Correction, 2026-08-04: the line this replaced claimed "ADR-053's stopgap — `content_post_delete` excluded from the wired tool surface — stays in force until Decision 2 lands." That was false in two independent ways, caught while implementing Decision 2 rather than before writing it. First, ADR-053 Decision 6 gated the tool "until Decisions 1–4 ship," not until *this* ADR's Decision 2 — and ADR-053's own Decisions 1–4 shipped as commit `61edab4`, well before this ADR was drafted. Second, and more directly: `content_post_delete` was never excluded from the wired tool surface at any point this codebase's history covers. `src/features/post/tool-registrations.ts`'s own comment (near its `buildDomainRegistrations` call) says so explicitly — "No `unwiredToolIds`: Posts/Pages wires its ENTIRE catalog" — and the catalog has always included the delete tool. Decision 2 has since landed (this dispatch): the tool now uses the held-open exchange shape described above instead of ADR-053 Decision 3's token redemption.
