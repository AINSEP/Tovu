...
- SPEC-003's install-dir compatibility stamp must identify the runtime schema by the latest migration's **tag** (identity), not just the migration **count** — two runtime builds can share a count but bundle different migrations at that index (a fork or a hotfix branch). Count-only comparison would falsely pass the "is this site newer/divergent?" guard. SPEC-003 REQ-05 records both `schemaVersion` (index) and `schemaTag` (identity) in `.site-meta.json` (Red-Team RT-005). This matters once the desktop host (ADR-011) distributes multiple runtime builds.
