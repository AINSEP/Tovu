...
- local → `https://localhost:4321` (a second local site, or a staging container) is **denied by default**. (Corrected 2026-09-18: this example read `http://` as drafted. Task 10 hard-requires `https` **even on loopback**, in `peer-url.ts` AND in the egress policy's `allowedSchemes`, because the peer credential is a bearer header on every request — an `http://` peer URL is refused outright, which is a separate refusal from the private-address one described next.) The named capability is `EgressPolicy.devHostAllowlist`, matched against the bracket-stripped hostname. A peer pointing at a private address must be explicitly allowlisted; the failure must say so rather than reading as a network error.
...
| 15 | **Post authorship now exists** (`posts.created_by_principal_id`/`created_at`, nullable, write-once — 2026-09-18, independent of this feature). Content-hash canonicalization (Task 1) must exclude both, same as `id`/`workspaceId`/`version`/`updatedAt`/`autosaveJson` — they are per-instance provenance, not content. The importer (Task 5/8) must copy the source bundle's values onto the destination row verbatim on `created`; an `applied`/`forced` update must never touch them, exactly mirroring `updatePost`'s own write-once contract. **Do not let an importer re-stamp every post with the importing operator's id.** | 1 | A `created` row's author/`createdAt` equal the source's; an `applied` update leaves the destination's unchanged even though the bundle carries different values; hashing two entities that differ only in these two fields produces the same hash. |
