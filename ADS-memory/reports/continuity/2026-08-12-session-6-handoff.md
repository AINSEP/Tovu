...
4. **The external audit never dispatched.** Packet is frozen and complete
   (`PKT-TOVU-20260812T124425Z`, `TM-TOVU-2026-08-12-B`, round 1, floor 8.5, 26 files, target
   `df109b5^..800bf20`). **Resume with
   `reuse_packet=ADS-memory/reports/external-audit/packets/20260812T124425Z-audit-packet.md`** rather
   than rebuilding — the threat model is frozen, and a rebuild would no longer be round 1.

   *Corrected 2026-08-12 after this handoff was written:* the paths above originally pointed at
   `ADS-memory/.local-artifacts/` (**gitignored** — one `git clean -xdf` from gone) and at
   `<scratchpad>/audit-stage/` (**session-scoped** — a later session resolves that to its own empty
   directory and finds nothing), which made "resume, don't rebuild" unachievable as written. Packet and
   both auditor prompts are now committed under
   `ADS-memory/reports/external-audit/dispatch-stage/20260812T124425Z/` — start from its `README.md`.
   `agy`'s 15-file stage is regenerated by the `restage-files.sh` there, verified byte-identical to the
   original stage. **Stage from `800bf20`, not `b658e1d`:** the packet's scope table lists `b658e1d`
   last, but `800bf20` landed seven minutes after it and is the real tip — staging from `b658e1d`
   silently serves the four `src/server/routes/site/*.ts` files in their pre-Cache-Control state.
   One packet defect disclosed there rather than fixed: the scope is headed "9 commits" and names 10.
