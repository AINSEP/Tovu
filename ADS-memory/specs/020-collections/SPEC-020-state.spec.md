...
| `PUBLISH_ENTRY` | `{id, expectedVersion}` | `expectedVersion` matches; owning `ContentType.status != 'tombstone'` (REQ-28) — **no precondition on the entry's current `status`; idempotent regardless of whether it is already `published`** (see note below) | `status → 'published'`, `publishedAt = now`, appends `EntryRevision` (`op='publish'`), stamps watermark, enqueues `entry.published` outbox event — unconditionally, even when `status` was already `published` (no `current.status === target.status` short-circuit) | rejects with `CONTENT_TYPE_NOT_ACTIVE` if owning type is `tombstone`; otherwise rejects only on version conflict |
| `UNPUBLISH_ENTRY` | `{id, expectedVersion}` | `expectedVersion` matches; owning `ContentType.status != 'tombstone'` (REQ-28) — **no precondition on the entry's current `status`; idempotent regardless of whether it is currently `published`** (see note below) | `status → 'unpublished'`, appends `EntryRevision` (`op='unpublish'`), stamps watermark, enqueues `entry.unpublished` outbox event — unconditionally, even when `status` was `draft` or already `unpublished` | rejects with `CONTENT_TYPE_NOT_ACTIVE` if owning type is `tombstone`; otherwise rejects only on version conflict |

> **Note (2026-09-03 owner ruling):** an earlier draft of this table gated `PUBLISH_ENTRY` on
> `status in {draft, unpublished}` and `UNPUBLISH_ENTRY` on `status='published'`, rejecting an
> already-published (or not-currently-published) entry. That contradicted this spec's own AC-45
> (`SPEC-020-traceability.spec.md`) and the `ENTRY_PUBLISH`/`ENTRY_UNPUBLISH` error mapping in
> `SPEC-020-api.spec.md` §6, neither of which defines an error code for "already published" or "not
> currently published" — and it did not match the shipped implementation
> (`transitionEntryStatus`, `Jini/packages/cms/src/entries/write-service.ts:323-359`), which has no
> `current.status === target.status` guard. AC-45/api.spec.md are the governing contracts:
> `PUBLISH_ENTRY`/`UNPUBLISH_ENTRY` are idempotent state assertions ("guarantee this entry ends up
> published/unpublished"), not a strict finite-state-machine transition — a repeat call (retry,
> double-click, queue redelivery) succeeds and re-runs the full effect set rather than erroring.
> This row above is corrected to match; do not reintroduce the rejecting guard.
