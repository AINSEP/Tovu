...
- **v0.1.2 (2026-09-13, drift fix).** The desktop main process was renamed from `apps/desktop/main.js` to `apps/desktop/main.ts` (commit `95206dd9`), and its tripwire test moved (also renamed) from `apps/desktop/src/renderer/one-chat-fab-wiring.test.js` to `apps/desktop/src/renderer/one-chat-fab-wiring.test.ts`. Updated every reference to those files (Problem Statement, Success signal, E2, E4, E10, REQ-12, Delivery Plan Phase 1, AC-18, Constitution Article II, Agent Directives) to the current names. E19 is superseded — the rename it recorded as pending has since happened — and is annotated rather than deleted. Reworded AC-18: it previously said `one-chat-fab-wiring.test.js`'s "existing assertions still pass unmodified," which conflicted with this spec's own Delivery Plan (Phase 1 explicitly flips that file's tripwire assertion on purpose when the `ipcMain.handle` registrations land, per the test's own comment). AC-18 now distinguishes the one-chat-rule assertions (still hold) from the tripwire assertion (intentionally updated in the same phase). No REQ, AC, or INV changes in substance.
...
| E2 | The renderer already calls all 5 invoke channels and subscribes to both push channels; nothing on the main-process side answers any of them | `apps/desktop/src/preload/preload.mts:97-105`; no `ipcMain.handle`/`ipcMain.on` for any of these channels exists in `apps/desktop/main.ts` (grep confirms zero matches) |
...
| E4 | The owner's one-chat rule is enforced by a regression test whose own comment states the intended future shape: a **panel** reachable from this app's own chrome, never a second floating FAB | `apps/desktop/src/renderer/one-chat-fab-wiring.test.ts:1-16` |
...
| E19 | SUPERSEDED (2026-09-13): as of 2026-09-12 `apps/desktop/main.js` was still the actual Electron main-process entry point, not yet renamed to `.ts`, and the ongoing JS→TS pass elsewhere in this app had not touched that file's name. It has since been renamed to `apps/desktop/main.ts` (commit `95206dd9`, "type main.ts and its wiring tests, batch B8"). Every other `main.js` reference in this spec (Problem Statement, E2, E10) now reads `main.ts` accordingly — this row is kept, not deleted, as the record of when and why the rename happened. | direct `ls`/`git log` against the working tree, 2026-09-13 |
...
- The one-chat rule: an embed signal that hides the admin's own `ChatFab`/`AssistantDock` inside the desktop shell, keeping `one-chat-fab-wiring.test.ts` passing (REQ-12).
...
- **REQ-12:** The admin app exposes a dedicated embed signal (e.g. `window.tovuDesktopEmbedded`, following the `window.tovuVoice` precedent) that its own `ChatFab`/`AssistantDock` checks at mount to hide itself when running inside the Tovu desktop shell's `<webview>`. This must keep every existing assertion in `one-chat-fab-wiring.test.ts` passing.
...
- This phase alone flips `one-chat-fab-wiring.test.ts`'s tripwire — expected, per that test's own comment.
...
- **AC-18 (REQ-12) [P1]:** Given the admin page is loaded inside the Tovu desktop shell's `<webview>`, when it mounts, then `ChatFab`/`AssistantDock` does not render, and `one-chat-fab-wiring.test.ts`'s one-chat-rule assertions (no `chat-fab` class in `App.tsx`, no `ChatFab` import, no `.chat-fab--disabled` style, `WorkspaceChatPane` still unreferenced as JSX) still hold. Its separate tripwire assertion (no `ipcMain.handle` for `WORKSPACE_CHAT_CHANNELS` in `main.ts`) is expected to flip in the same phase that lands REQ-10's handlers (Delivery Plan Phase 1) and is intentionally updated then, per the test's own comment — that is not a regression.
...
| II — Test-First | COMPLIES | REQ-05 (tool-set fixed at turn start), REQ-11 (confirmation), REQ-19 (site-down degradation), and the `one-chat-fab-wiring.test.ts` flip all need RED tests before implementation, per the Delivery Plan's phase boundaries. |
...
- Reintroduce a second floating chat FAB — `one-chat-fab-wiring.test.ts` exists specifically to catch that regression.
