...
| version | 1.2.0 |
| status | DRAFT |
| content_hash | sha256:5a9559407323ddb564ce3c57eea1093e496b58e2688cf8701c423ba59f9af7ae |
...
**Amendment v1.1.0 (2026-07-29, B1 fix — awaiting owner sign-off):** REQ-06 is amended. The
original "exactly one workspace row, else `SITE_CORRUPT`" contract was itself a real regression:
SPEC-044's admin `CREATE_WORKSPACE` route can create a 2nd row through normal, permission-gated
use, which then bricked boot on every subsequent restart with no recovery path via any API. Owner
decision: workspaces are a real, intentional multi-site tenancy primitive (ADR-007), not an error
condition. `resolveWorkspace`/`bootSiteDir`/`tovu serve` now tolerate >1 rows, defaulting to the
oldest and accepting an explicit `--workspace <id>` (new REQ-06b) to pick another. Zero rows
remains the one genuine corruption state. See `state.spec.md` §5/§6 and `api.spec.md`'s `CLI_SERVE`
contract for the full amended text.

**Amendment v1.2.0 (2026-07-29, awaiting owner sign-off):** new REQ-11, `tovu introspect`. This
CLI is meant to be driven programmatically (Tovu-Runner), and `api.spec.md` — a hand-maintained
doc — had already silently drifted once (documented `--workspace` after it existed in code). REQ-11
closes that gap by exposing the CLI's own live `commander` `Command` tree as JSON, plus an
MCP-tool-definition reshaping, so an agent driving `tovu` reads the real schema instead of trusting
a doc that can go stale.

Status reverted to DRAFT per this project's standing amendment convention (spec amendments always
need a fresh checkpoint) — content_hash needs Spec-Agent-mechanical recomputation once these
amendments are reviewed together.
...
- REQ-06: The serving workspace id is resolved from `content.db`: zero rows abort with `SITE_CORRUPT` (exit 5). **Amended 2026-07-29 (B1 fix):** more than one row no longer aborts — a site MAY grow additional workspace rows post-init via SPEC-044's admin `CREATE_WORKSPACE` route (ADR-007: workspaces are a real multi-site tenancy primitive, not an error condition). `tovu serve` defaults to the oldest row (by `createdAt`) and accepts an explicit `--workspace <id>` (REQ-06b) to pick another; an id matching no row is `VALIDATION` (exit 2). The hardcoded seeded-workspace id in `server/deps.ts` is removed from the install-dir path.
- REQ-06b (added 2026-07-29, B1 fix): `tovu serve <dir> --workspace <id>` selects exactly that workspace id instead of the default (oldest). Omitting `--workspace` on a single-workspace install is unaffected (byte-identical to pre-amendment behavior).
...
- REQ-11 (added 2026-07-29): `tovu introspect [--format commander|mcp]` prints a JSON description of this CLI's own commands/arguments/options, read live off the `commander` `Command` tree (`src/cli/introspect.ts`) rather than a hand-maintained doc — so it cannot drift from what `init`/`serve` actually accept. `--format mcp` reshapes the same data into MCP tool definitions (`{name, description, inputSchema}`, one per command, `name` prefixed `tovu_`) for an agent (e.g. Tovu-Runner) invoking `tovu` as a subprocess. Any `--format` other than `commander`/`mcp` is `VALIDATION` (exit 2).
...
Note: `ADS-memory/governance/constitution.md` is still not bootstrapped; toolkit default articles applied (same as SPEC-001/002). Flagged to Coordinator.
