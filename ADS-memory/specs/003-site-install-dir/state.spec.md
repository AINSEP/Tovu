...
| `resolveWorkspace` | open db, optional `workspaceId` | the resolved `WorkspaceRecord` | 0 rows ⇒ `SITE_CORRUPT`; >1 rows + no `workspaceId` ⇒ the OLDEST row (by `createdAt`), with a logged warning; `workspaceId` given but no row matches ⇒ `VALIDATION` (amended 2026-07-29, B1 fix — see §6 note) |
...
- The `workspaces` table of a v1 site contains at least one row from init onward. **Amended 2026-07-29 (B1 fix):** a site MAY grow additional workspace rows post-init via SPEC-044's admin `CREATE_WORKSPACE` route — this is intentional (ADR-007: workspaces are a real multi-site tenancy primitive), not corruption. `tovu serve` defaults to the oldest row and accepts `--workspace <id>` to pick another. The original text ("contains exactly one row... multi-workspace sites are not supported in v1") was itself the bug: it made a normal, permission-gated admin action (creating a 2nd workspace) brick boot on every subsequent restart, with no recovery path. Only a zero-row table remains a genuine corruption state.
