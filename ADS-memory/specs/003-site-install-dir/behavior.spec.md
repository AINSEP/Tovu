...
- BR-05: WHEN serving, the system shall evaluate in this order, first failure wins: (1) dir readable + `config.json` valid (`SITE_DIR_INVALID`), (2) `.site-meta.json` present + valid (`SITE_DIR_INVALID`), (3) `content.db` present + openable — **absent** ⇒ `SITE_DIR_INVALID`; **present but not a valid SQLite database** (truncated/garbage) or **locked by another process** ⇒ `SITE_CORRUPT` (RT-002), (4) schema guard (`SITE_NEWER_THAN_RUNTIME`), (5) forward migration when older, then the `schemaVersion` + `schemaTag` stamp update (both fields together), (6) workspace resolution (`SITE_CORRUPT`), (7) listener bind (`PORT_IN_USE`).
- BR-06: The system shall update `.site-meta.json.schemaVersion` **and** `.site-meta.json.schemaTag` — writing both together in one stamp — only AFTER Drizzle `migrate()` completes successfully (applying any pending generated migrations from `drizzle/`), setting them to the runtime's bundled-migration identity (`schemaVersion` = index, `schemaTag` = tag/hash; BR/RT-005), and shall never lower `schemaVersion` (INV-05, EC-09). The system shall never leave `schemaVersion` bumped with a stale `schemaTag`; if the stamp write cannot complete both fields, it must write neither (the migration itself remains idempotently re-appliable via the `__drizzle_migrations` journal on the next serve).
- BR-07: WHEN the process receives SIGINT/SIGTERM while serving, the system shall stop accepting new connections, allow the current synchronous request to return before closing the db handle (better-sqlite3 is synchronous, so at most one request is mid-flight and it either completed or had not started), then close the handle and exit 0 (RT-004).
...
| `host` (serve) | `127.0.0.1` | `TOVU_HOST` env overrides; IP literal only; `::` = all interfaces |
...
- BR-02a (bind host, LAN-bind plan 2026-09-23): WHEN resolving the serve bind host, the system shall use `TOVU_HOST` when set and non-blank (an IP literal per `net.isIP`; `::` resolves to Node's own all-interfaces default), else `127.0.0.1`. A non-IP-literal value (e.g. a hostname) is `VALIDATION`, exact text: `TOVU_HOST must be an IP address such as 127.0.0.1 or 0.0.0.0 (got "<value>")`.
...
- IF an init filesystem write fails mid-flight (`ENOSPC` disk full, `EACCES`/`EROFS` permission), THEN init shall exit `INTERNAL` (exit 1) after best-effort cleanup; if cleanup itself fails, the message shall name the partial dir for manual removal — and because the commit marker (`.site-meta.json`) is never reached, `serve` still refuses the partial dir, so INV-02 holds even under failed cleanup (EC-10, RT-003).
