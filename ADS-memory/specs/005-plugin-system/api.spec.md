...
- Version: `1.1.2` (adds an additive `tier` field to `PLUGINS_LIST`'s response per REQ-10's 1.1.2
  amendment — see `feature.spec.md`'s `revision_note`; all other content byte-unchanged from `1.1.0`)
- Content Hash: `sha256:see feature.spec.md (package hash of record)`
- Last Edited: `2026-07-28T00:00:00Z` (adds `tier` to `PLUGINS_LIST`'s response contract, §5)
...
|---|---|---|---|---|---|
| `PLUGINS_LIST` (new) | `GET` | `/api/admin/v1/workspaces/:workspaceId/plugins` | List discovered plugins with source/tier/status/enabled/errors | `AUTH_LOCAL_DEV` | `NONE_LOCAL` |
...
plugins:
  ...
  items:
    ...
    tier: enum[tier-1, tier-2, tier-3]   # (1.1.2) additive — projected verbatim from PluginManifest.tier (REQ-01/ADR-024 §1)
