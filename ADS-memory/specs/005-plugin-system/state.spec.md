...
- Version: `1.1.2` (adds `PluginDiscoveryRecord.tier`, projected from `PluginManifest.tier`, so
  `PLUGINS_LIST` can expose it per REQ-10's 1.1.2 amendment — see `feature.spec.md`'s `revision_note`;
  all other content byte-unchanged from `1.1.1`)
- Content Hash: `sha256:see feature.spec.md (package hash of record)`
- Last Edited: `2026-07-28T00:00:00Z` (adds `tier` field to `PluginDiscoveryRecord`)
...
PluginManifest:                      # tovu.plugin.json — the ecosystem compatibility surface (ADR-004)
  ...
  tier: enum[tier-1, tier-2, tier-3] # plugin trust tier (ADR-024 §1); v1 loader executes Tier-3 (in-process
                                     # ESM) only — v1 manifests, incl. word-count (REQ-09), declare "tier-3"
...
PluginDiscoveryRecord:               # in-memory, rebuilt each discovery (NOT persisted) — feeds PLUGINS_LIST
  ...
  tier: enum[tier-1, tier-2, tier-3] # (1.1.2) projected verbatim from PluginManifest.tier (ADR-024 §1);
                                     # feeds REQ-18's list-row badge (ui.spec.md §5)
