...
- Version: `1.1.2` (content byte-unchanged from `1.1.1`; version stamp carried forward with the
  package per the 1.1.2 tier-badge/REQ-18 fix — see `feature.spec.md`'s `revision_note`. Displaying an
  already-validated field on an existing list screen introduces no new error path — no new error code
  was needed)
- Content Hash: `sha256:see feature.spec.md (package hash of record)`
- Last Edited: `2026-07-28T00:00:00Z` (content unchanged; package-level bump)
