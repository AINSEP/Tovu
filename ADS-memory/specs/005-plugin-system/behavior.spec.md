...
| version | 1.1.2 (content byte-unchanged from 1.1.1; version stamp carried forward with the package per the 1.1.2 tier-badge/REQ-18 fix — see feature.spec.md's `revision_note`) |
| content_hash | sha256:see feature.spec.md (package hash of record) |
| last_edited | 2026-07-28T00:00:00Z (content unchanged; package-level bump for the 1.1.2 REQ-18/tier-badge amendment) |
...
| `manifest.tier` absent or not one of `tier-1`/`tier-2`/`tier-3` | reject (`MANIFEST_MALFORMED`) | Required trust-tier declaration (ADR-024 §1); no safe default — silently defaulting could misrepresent a plugin's actual trust tier |
