...
- **(0.7.0)** An HTTP route for `CREATE_PRINCIPAL` (`APIKEY_PRINCIPAL_CREATE`, api.spec §1) — the
  transition was specified at v0.5.1 but had no surface, which left `ISSUE_API_KEY` with no reachable
  way to obtain the `kind='api_key'` binding target it requires — REQ-08 (unchanged transition, new
  route; AC-33)
...
- AC-33 (REQ-08/REQ-01 route reachability) [P1] **(0.7.0)**: Given a caller holding `apikey.manage`,
  when it calls `APIKEY_PRINCIPAL_CREATE` with `{ displayName: "ci-bot" }`, then a new principal is
  created with `kind='api_key'`, `status='active'`, the caller's own `workspaceId`, and **zero**
  `principal_roles` and `principal_policies` rows, and the response is `201` carrying that principal
  — and that principal id is immediately accepted as `APIKEY_ISSUE`'s bound `principalId`, so the
  mint→issue path is reachable end-to-end over HTTP (the gap ADR-PIPE-006 Decision 3 raised). Given
  the **same** call with `kind` set to `'agent'`, `'user'`, or `'system'`, then it is rejected 400
  `VALIDATION_ERROR` and no principal is created — `CREATE_PRINCIPAL` mints machine identities only
  (`agent` deferred, OQ-01; humans via `CREATE_USER` per AC-22; `system` via seed per REQ-09), so
  this route can never become a second creation path for a privileged kind. Given a caller **without**
  `apikey.manage`, then it is denied 403 `FORBIDDEN` and no principal is created. Given a blank
  (after-trim) `displayName`, then it is rejected 400 `VALIDATION_ERROR`.
...
| VII — Spec Integrity | COMPLIES | All package files reference spec_id SPEC-006 and version 0.7.0; feature.spec carries the canonical content_hash (VII / DoD G-07). **(0.7.0)** Resolves the one Article VII EXCEPTION ADR-PIPE-006 declared: `CREATE_PRINCIPAL` was an unreachable transition (specified in state.spec §3, absent from api.spec.md). The Architect surfaced it as `[NEEDS CLARIFICATION]` instead of inventing a contract; this amendment ratifies the shape, so the exception is closed at the spec layer and the ADR's contingent `create-principal.ts` unit is unblocked. |
...
- [x] spec_id assigned and unique (SPEC-006, verified against `ADS-memory/reports/pipeline/` and specs/)
- [x] version set to correct semver (0.7.0 — minor: one additive endpoint + AC-33, no change to the
      security core; 0.6.0's additive CRUD-completion scope carries forward unchanged)
- [ ] status set to APPROVED (not DRAFT or REVIEW) — **REOPENED at 0.6.0, still open at 0.7.0.** The
      pre-existing v0.5.6 content (through AC-26/INV-08) keeps its 2026-07-09 APPROVED provenance and
      is unchanged. The **new** 0.6.0 material (REQ-15..19, AC-27..32, INV-09, EC-14..17) and the
      **new** 0.7.0 material (`APIKEY_PRINCIPAL_CREATE` + AC-33, plus the api.spec §1 path-prefix
      drift repair) have **not** been through Red-Team or an owner DRAFT→APPROVED checkpoint — both
      amendments are Spec Agent output only. Coordinator: route both to Red-Team in one pass before
      Software Architect dispatch, same as the original v0.4.1→v0.5.0 path. 0.7.0 adds no new
      escalation surface (the new endpoint writes no grant rows and is clamp-exempt for that reason),
      but it is a **credential-adjacent** route and should be read as such by Red-Team.
...
APPROVED-quality). The **0.6.0 and 0.7.0 amendments are DRAFT**, pending the same two-step checkpoint
v0.5.6 itself went through: (1) one Red-Team pass covering both REQ-15..19/AC-27..32/INV-09/EC-14..17
(0.6.0) and `APIKEY_PRINCIPAL_CREATE`/AC-33 (0.7.0), (2) owner DRAFT→APPROVED sign-off. Not cleared
for Software Architect dispatch until both land. This spec package is otherwise complete and
internally consistent (see spec-dod.md's 0.6.0/0.7.0 sections for the itemized status) — the only gap
is the approval ceremony for the new material, exactly the shape the 0.5.x gate was in before
2026-07-09.

**0.7.0 note on what this gate does *not* block.** `[NEEDS CLARIFICATION]` markers block Software
Architect dispatch; the DRAFT status blocks it too. But ADR-PIPE-006 was already written against
0.6.0-DRAFT under a targeted remediation dispatch, and its sole spec-layer blocker was the
`CREATE_PRINCIPAL` contract, which is now ratified. So the ADR's `create-principal.ts` unit is no
longer blocked *on Spec*; it remains blocked on the ADR's own PROPOSED→ACCEPTED human sign-off and on
this status gate, which are separate ceremonies with separate owners.
