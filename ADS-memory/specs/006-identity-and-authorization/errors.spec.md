...
**0.7.0 note:** No new error codes. The new `APIKEY_PRINCIPAL_CREATE` endpoint (api.spec §1 — the
`CREATE_PRINCIPAL` surface) emits only `VALIDATION_ERROR` (new emission site: blank/oversize
`displayName`, or a `kind` other than `api_key`), `UNAUTHENTICATED`, `FORBIDDEN`, and the blanket
`RATE_LIMIT_EXCEEDED`. It notably cannot emit `GRANT_EXCEEDS_ISSUER` — it writes no grant rows, so
the INV-07 clamp has nothing to bound (api.spec §1 Shape Rationale, §6).
...
| `VALIDATION_ERROR` | API request validators + grant/issuance preconditions (REQ-08/REQ-02) | API + UI + CLI | Malformed login / issuance bodies (UI maps `fieldErrors`); **(0.7.0)** `CREATE_PRINCIPAL` with a blank/oversize `displayName` or a `kind` other than `api_key` — incl. the deferred `agent` (AC-33, OQ-01); also: `ISSUE_API_KEY` bound principal not `kind='api_key'` (AC-23) or not **grantless** (AC-25a); `ASSIGN_ROLE`/`ATTACH_POLICY` target not `kind='user'` (AC-25b); `WRITE_POLICY_PERMISSION` against an `is_builtin` or `is_frozen` (issuance-snapshot) policy is refused (AC-26) — machine authority is set at a single clamped issuance and frozen thereafter (F-053-01/F-054-01); **(0.6.0)** `ENABLE_PRINCIPAL` against a non-`kind='user'` target (AC-27); `UPDATE_ROLE`/`UPDATE_POLICY`/`DELETE_ROLE`/`DELETE_POLICY` against an `is_builtin` (or, for policies, `is_frozen`) target (AC-30/AC-31) |
