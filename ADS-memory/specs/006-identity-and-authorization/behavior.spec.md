...
| `display_name` length (`principals`) | 1–255 characters (post-trim) | API | **(0.7.0)** Blank-only rejected `VALIDATION_ERROR` at `APIKEY_PRINCIPAL_CREATE` (AC-33). Matches `username`/`label`; **not** unique-constrained — a principal has no login identity to collide with (§5.1 dedup covers `username` only). |
...
| `CREATE_PRINCIPAL` with `kind` set to `'agent'`, `'user'`, or `'system'` (0.7.0) | THEN it is rejected 400 `VALIDATION_ERROR` and no principal is created; `APIKEY_PRINCIPAL_CREATE` mints `kind='api_key'` only — `agent` is deferred (OQ-01), humans come from `CREATE_USER` (AC-22), `system`/`user-local` from `SEED_FIRST_BOOT` (REQ-09) — so this route can never become a second creation path for a privileged kind — REQ-08/REQ-01, AC-33. | Yes |
| `CREATE_PRINCIPAL` by a caller lacking `apikey.manage` (0.7.0) | IF the caller does not hold `apikey.manage`, THEN it shall return 403 `FORBIDDEN` and no principal is created. The transition writes no grant rows, so the INV-07 clamp does not apply and `GRANT_EXCEEDS_ISSUER` is never emitted here — the permission gate is the whole check — REQ-08, AC-33. | Yes |
