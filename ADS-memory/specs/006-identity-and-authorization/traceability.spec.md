...
**Purpose:** Traces every REQ/AC/INV/EC from `feature.spec.md` v0.7.0 to its implementation and
...
AC-27..32/INV-09/EC-14..17 rows below are this amendment's responsibility. **0.7.0 note:** adds
AC-33 only (the `CREATE_PRINCIPAL` HTTP surface). It introduces no new REQ/INV/EC — the transition,
its preconditions, and its failure handling were already specified at state.spec §3; only the route
was missing, so AC-33 attaches to the existing REQ-08/REQ-01 pair.
...
| AC-33 (REQ-08/REQ-01 route reachability) (0.7.0) | `APIKEY_PRINCIPAL_CREATE` mints a grantless, active `kind='api_key'` principal in the caller's workspace and its id is accepted by `ISSUE_API_KEY` (mint→issue reachable end-to-end); non-`api_key` `kind` → VALIDATION_ERROR; missing `apikey.manage` → 403; blank `displayName` → VALIDATION_ERROR | P1 | pending | pending | pending | pending | PENDING |
...
All REQ-01..19, AC-01..33, INV-01..09, and EC-01..17 from feature.spec.md appear in Sections 1–3.
...
| Spec Agent | Spec Agent (Direct dispatch) | 2026-07-28 | Extended for v0.7.0 `CREATE_PRINCIPAL` route ratification (AC-33 only — no new REQ/INV/EC). Resolves ADR-PIPE-006 Decision 3's `[NEEDS CLARIFICATION]`; row PENDING pre-TDD. Rolls into the same Red-Team + owner checkpoint 0.6.0 already owes. |
| TDD Agent | | | |
| Programmer Agent | | | |
| Code Review Agent | | | |
| Coordinator | | | |
