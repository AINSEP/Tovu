...
**Implementation commits:** 6307f04f (M1 folder drop), 7896d028 (M2 agent guidance), a9395a7a (M3 binary assets); review fixes 10871375 (bridge limited to /admin), 32316495 (plugins_set_enabled guidance), 8f71c429 (fs tools findable by search), Jini 559e233f (stuck drop outline); 64b11593 (admin folder drop moved onto `@jini-ai/ui` folder-path-drop, Jini 59acc4db; admin `folder-drop.ts` deleted).
...
|-----------|-------------|----------|-----------|---------------|-----------|---------|--------|
| REQ-01 | Folder drop inserts path, not attachments | — | admin/features/fs-files/hooks/use-folder-drop.hooks.ts; Jini packages/ui/src/features/folder-path-drop/rules.ts (@ Jini 59acc4db); desk/speech/preload-speech.cts | `useFolderDrop.handleDropCapture` (via `useFolderPathDropCapture`); `folderPathsFromDataTransfer`, `captureFolderPathDrop`, `formatDroppedFolderPaths`; `window.tovuFiles` | admin/features/fs-files/hooks/__tests__/use-folder-drop.hooks.unit.test.ts; Jini packages/ui/src/__tests__/features/folder-path-drop/rules.test.ts; desk/speech/preload-speech.test.ts | "inserts the path and sets the custom root on a successful drop (AC-01/AC-02)"; "recovers the OS path for a single dropped folder" (@ Jini 59acc4db); "window.tovuFiles is exposed on the admin surface" | VERIFIED (unit) |
...
| EC-02 | User drops a file, not a folder | use-folder-drop.hooks.unit.test.ts; Jini packages/ui/src/__tests__/features/folder-path-drop/rules.test.ts (@ Jini 59acc4db) | "falls through (no-op) when nothing dropped is a folder (EC-02, a loose file)"; "skips a file item whose entry is not a directory" | VERIFIED |
