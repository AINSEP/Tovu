...
test("main.js imports registerSpeechIpc from speech-ipc.js", () => {
  assert.match(source, /from ["']\.\/src\/speech\/speech-ipc\.js["']/);
  assert.match(source, /registerSpeechIpc/);
});

test("main.js calls registerSpeechIpc with the real ipcMain before attach mode's window can load", () => {
  const registerCallIndex = source.indexOf("registerSpeechIpc(");
  const attachBranchIndex = source.indexOf("if (attachUrl)");
  assert.notEqual(registerCallIndex, -1, "expected a registerSpeechIpc(...) call in main.js");
  assert.notEqual(attachBranchIndex, -1, "expected the attach-mode branch in main.js");
  assert.ok(
...
test("createWindow's webPreferences names a preload script", () => {
  ...
  assert.notEqual(createWindowStart, -1, "expected a createWindow function in main.js");
