...
import { validateManifest, detectGateDrift, formatSummary, isFailure, daysBetween } from "./quality-gates.js";
...
test("a disabled gate with a reason and a fresh date is accepted — turning a gate off is allowed", () => {
  const problems = validateManifest(
    manifest([{ id: "coverage", run: "node scripts/check-coverage.mjs", enabled: false, disabledReason: "baseline pending", disabledOn: "2026-09-10" }]),
    TODAY
  );
  assert.deepEqual(problems, []);
});
...
test("a gate script on disk that no manifest entry runs is detected", () => {
  const drift = detectGateDrift(
    [{ id: "coverage", run: "node scripts/check-coverage.mjs" }],
    ["check-coverage.mjs", "check-complexity.mjs"]
  );
  assert.deepEqual(drift, ["check-complexity.mjs"], "the unwired script must be named");
});

test("a DISABLED gate still counts as registered — off is a decision, absent is not", () => {
  const drift = detectGateDrift(
    [{ id: "coverage", run: "node scripts/check-coverage.mjs", enabled: false }],
    ["check-coverage.mjs"]
  );
...
test("a run command carrying flags still matches its script", () => {
  const drift = detectGateDrift(
    [{ id: "c", run: "node scripts/check-coverage.mjs --areas js,ts" }],
    ["check-coverage.mjs"]
  );
  assert.deepEqual(drift, []);
});
