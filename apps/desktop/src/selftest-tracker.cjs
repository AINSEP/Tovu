/**
 * @file Tracks window load outcomes for `TOVU_DESKTOP_SELFTEST=1`, split out of `main.cjs` so the
 ...
 * pure completion tracker and `main.cjs` keeps the only code that actually touches Electron or the
 * process.
 *
 ...
 *    (`loadURL` is fire-and-forget, and `main.cjs` opens each site in its startup loop sequentially).
 ...
 *    attached after the fact. `main.cjs`'s `createWindow` therefore calls this tracker's `add()` the
...
module.exports = { createSelftestTracker };
