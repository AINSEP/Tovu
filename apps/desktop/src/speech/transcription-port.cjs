 ...
 * `mac-on-device-transcriber.cjs` for the only implementation that exists today, and
 * `speech-ipc.cjs` for the one caller that resolves a port and puts it behind two IPC handlers.
 *
 ...
 *   network when on-device recognition is unavailable — see `mac-on-device-transcriber.cjs`'s own
...
module.exports = { resolveTranscriptionPort, unavailablePort };
