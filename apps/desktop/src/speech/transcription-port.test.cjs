/**
 * @file Direct tests for `transcription-port.cjs`'s platform-selection logic. No Electron, no real
 * macOS port — `createMacPort` is a plain injected factory here.
 */
const test = require("node:test");
const assert = require("node:assert/strict");

const { resolveTranscriptionPort, unavailablePort } = require("./transcription-port.cjs");
