 ...
 * is nothing here". (`runner:projects:list`/`create`/`delete`/`open-external`/`start` are no
 ...
 * `src/speech/preload-speech.cjs` makes. `runner-ipc-stubs.test.cjs` parses the contract sources
...
const RUNNER_STUB_CHANNELS = Object.freeze([
  ...
  // (`project-ipc.cjs`, registered in `main.cjs` before this module runs). `stop` stays stubbed —
  // no control in the per-project bar calls it yet; see `RUNNER_PROJECT_CHANNELS.stop`'s own doc.
...
module.exports = { registerRunnerIpcStubs, notPortedError, RUNNER_STUB_CHANNELS, RUNNER_MAIN_NOT_PORTED };
