 ...
 * an object — no renderer, no component, no hook harness. It was the second half of the old
 * `useProjectTabs`; separating it is also what breaks the cycle that made `useSectionNav`
 * impossible, since the derivation needs `activeId`/`appearanceOpen` while `useSectionNav` needs
 * the tab setter.
 */
export function deriveFleetView(input: {
  ...
  // project tab, not over a non-Projects section. This mirrors the gate order `MainArea` used to
  // run itself before this fix (`appearanceOpen`, then `!showFleet`, then `activeId !== 'projects'`)
  // — reusing `showFleet`/`inProjects` here rather than re-deriving them is what keeps this
  // condition locked to that order instead of drifting into a second, slightly different rule.
...
/**
 * Immersive mode: the active project's admin takes the whole window and Runner's own chrome (top
...
export function useExpandedMode(showProjectTab: boolean): {
  ...
  // Escape collapses. This listener only sees keys pressed in Runner's own chrome — a <webview>
...
export function useProjectStart(project: ProjectRecord): {
  ...
  const start = async () => {
    const bridge = runnerInventoryBridge();
    if (bridge === undefined) {
      setError('Runner desktop connection required to start a website.');
      return;
    }
    setStarting(true);
    setError(null);
 ...
 * might be.
 *
 * This is the mid-session counterpart to `reconcile()` in the main process, which only rescues a
 * wedged sidecar at boot. A process that dies between polls, or never answers its port in the
 * first place, leaves its registry row at `running` — `ProjectWorkspace` renders the `<webview>`
 * on that basis, the guest fails to load, and without this hook nothing ever notices: the row
 * never changes, so the 4s poll that normally swaps a failed state for a working one has nothing
 * to swap.
 *
 ...
 *
 * The timeout exists because `did-fail-load` covers less than its own doc comment implies. Verified
 * live: `kill -STOP` on a sidecar leaves its listening socket still completing the TCP handshake —
 * the guest's request reaches an ESTABLISHED connection and then just sits there, since the kernel
 * accepted the bytes on the suspended process's behalf and nothing is left to read them. That is
 * not a failure Chromium's network stack ever reports; watched live for four and a half minutes,
 * `did-fail-load` never fired, and `did-finish-load` fired on its own the moment the process was
 * resumed — the same pending request the kernel had been holding finally got answered, with no
 * remount and no operator action. That recovery is exactly what a fresh `failed`-style swap would
 * have thrown away, which is why `stalled` gets its own, gentler branch.
 *
 * `STALL_TIMEOUT_MS` is this hook's own ceiling on "still loading, no verdict yet" — measured live
 * rather than guessed: a freshly created project's very first `/admin/` request, its slowest real
 * case (nothing has warmed that route, and `startProjectUnserialized`'s own readiness probe only
 * confirms `/`, never `/admin/`), took 808–1089ms across three separate projects on this machine —
 * one already-running project reopened after a renderer reload (1089ms) and two brand-new projects
 * on their first-ever open (837ms, 808ms). 8s is roughly 7x that slowest measurement: comfortably
 * past how long a local admin normally takes to answer, comfortably short of leaving the operator
 * staring at a blank pane for the rest of the session.
 *
