 ...
 *
 * Source text, because there is nowhere else to put it: `apps/desktop`'s test script runs this
 * file itself under `node --import tsx --test`, which transpiles but supplies no DOM — there is no
 * jsdom or testing-library in `devDependencies`, so a `.tsx` component cannot be rendered and
 * queried here. A missing link in this chain does not fail `npm run typecheck` either — an optional
