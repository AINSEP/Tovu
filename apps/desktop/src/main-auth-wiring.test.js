...
function startSiteBackendBody() {
  ...
  assert.notEqual(start, -1, "expected startSiteBackend in main.js");
...
test("the session decision is made AFTER the server is up, through ensureSiteSession", () => {
  const body = startSiteBackendBody();
  assert.match(body, /ensureSiteSession\(/, "main.js must use the extracted, behaviourally-tested decision");
  assert.ok(
...
test("main.js no longer treats bare cookie presence as a session", () => {
