...
function functionBody(name) {
  ...
  assert.notEqual(start, -1, `expected a function ${name}(...) in main.js`);
  const rest = source.slice(start);
  const end = rest.indexOf("\nfunction ", 1);
  return end === -1 ? rest : rest.slice(0, end);
}

test("main.js imports every site-preview-store operation it needs", () => {
  assert.match(source, /from ["']\.\/src\/site-preview-store\.js["']/);
