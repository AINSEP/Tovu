...
export const RUNNER_SECTIONS = [
  ...
  {
    // Label changed to 'Websites' (owner request, 2026-09-13: the top-nav tooltip should read
    // "Websites" rather than "Sites"). Was 'Sites' before that per an earlier vocabulary ruling
    // (a website project is a "site" — see Commit 4's ProjectRecord -> SiteRecord); that renaming
    // still lives on in code identifiers (`SiteRecord`, `openSites`, `inSites`, etc.), which this
    // change does not touch — only the user-facing label moves. Id stays `projects` deliberately:
    // RunnerSectionId values are a public contract in agent tool schemas and URLs (see this file's
    // header), same precedent as `id: 'generation'` keeping its id while only `label` moved to 'Media'.
    id: 'projects',
    group: 'workspace',
    label: 'Websites',
    agentDescription:
