/**
 * @file Static-analysis tests for `../main.cjs`'s shutdown wiring (D-09) — that a window's `closed`
 ...
 *
 * Source text, for the reason `main-speech-wiring.test.cjs` documents at length: `main.cjs`
 ...
 * (`shutdown-tracker.test.cjs`); what only this file can check is that `main.cjs` uses it, and in
...
function closedHandler() {
  ...
  assert.notEqual(start, -1, 'expected a window.on("closed", ...) handler in main.cjs');
...
test("closing a window only deletes the entry that window still owns", () => {
  // A crashed server is removed from `openSites` by `site-supervisor.cjs`, and the operator can
