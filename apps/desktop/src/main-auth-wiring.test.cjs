/**
 * @file Wiring guard for `../main.cjs`'s site-session sequence — DS-01.
 *
 * Source text, for the reason `main-speech-wiring.test.cjs` documents: `main.cjs` requires
 ...
 * `desktop-auth.cjs`'s `ensureSiteSession` precisely so it could be covered behaviourally, and it is
 * (four tests in `desktop-auth.test.cjs`). What only this file can check is that `main.cjs` reaches
...
function startSiteBackendBody() {
  ...
  assert.notEqual(start, -1, "expected startSiteBackend in main.cjs");
...
test("the session decision is made AFTER the server is up, through ensureSiteSession", () => {
  const body = startSiteBackendBody();
  assert.match(body, /ensureSiteSession\(/, "main.cjs must use the extracted, behaviourally-tested decision");
  assert.ok(
...
test("the probe is pointed at this site's own admin URL and its own partition", () => {
  ...
  // port, which is why `sitePartition` exists at all — see `desktop-auth.cjs` header, property 2).
  const body = startSiteBackendBody();
...
test("main.cjs no longer treats bare cookie presence as a session", () => {
