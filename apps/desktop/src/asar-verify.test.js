...
import { filesUnderPrefixes, formatMismatchReport, verifyAsarAgainstSource } from "./asar-verify.js";
...
test("a prefix naming a FILE directly (main.js) is included as-is, not walked", () => {
  const header = { "main.js": { size: 4, offset: "0" } };
  assert.deepEqual(filesUnderPrefixes(header, ["main.js"]), ["main.js"]);
});
...
test("a prefix absent from the archive is silently skipped, not an error", () => {
  ...
  assert.deepEqual(filesUnderPrefixes(header, ["src", "bin", "main.js"]), []);
});
...
test("the report names every mismatched file individually and explains the failure class", () => {
  const report = formatMismatchReport([{ relPath: "src/tracked-sites.js", reason: "content differs from source" }]);
  assert.match(report, /MISMATCH\s+src\/tracked-sites\.js/);
...
async function buildFixture() {
  ...
  const files = {
    ...
    "main.js": "// fixture main ".repeat(20) + "UNIQUE-MARKER-MMMM\n",
  };
...
test("a clean archive matches source on every file, byte-for-byte", async () => {
  ...
  try {
    const { checkedCount, mismatches } = verifyAsarAgainstSource(asarPath, root, ["src", "bin", "main.js"]);
...
test("a file present in the archive but deleted from source is reported as missing, not silently skipped", async () => {
  ...
  try {
    ...
    const { mismatches } = verifyAsarAgainstSource(asarPath, root, ["src", "bin", "main.js"]);
