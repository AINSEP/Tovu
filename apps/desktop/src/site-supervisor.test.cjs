/**
 * @file Coverage for `site-supervisor.cjs` — the owner of the `running -> exited` transition that
 * D-06 found had no owner at all.
 *
 * Every behavioural test here runs TWICE, against two stores built by `stores()`: the supervisor,
 * and a plain `Map` — the exact shape `main.cjs`'s `openSites` was before this module existed, with
...
test("the Map surface main.cjs and project-ipc.cjs already use behaves identically", () => {
  for (const { name, store } of stores()) {
