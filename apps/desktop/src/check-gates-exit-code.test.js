...
const RUNNER = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "scripts", "check-gates.mjs");
...
test("a gate script on disk that the manifest does not run fails the run", () => {
  ...
  assert.match(stderr, /check-orphaned\.mjs/);
...
test("a gate script the manifest DOES run is not reported as drift", () => {
  const { status } = runWith(
    { gates: [{ id: "cov", run: "node scripts/check-coverage.mjs" }, { id: "ok", run: "node -e \"process.exit(0)\"" }] },
    ["check-coverage.mjs"]
  );
