 ...
 *
 * Mirrors `src/speech/preload-speech.test.cjs` exactly — same regex-over-source approach, same
 * source of truth (`speech-ipc.cjs`'s own exports), for the same reason: two preloads now expose
 ...
 * a rename, and `src/runner-ipc-stubs.test.cjs` covers the main-process side.
 *
 * The bridging itself is deliberately not tested: `contextBridge.exposeInMainWorld` only runs
 * inside Electron's real preload context, exactly as `preload-speech.test.cjs`'s own header
 * records.
 */
...
const { IPC_CHANNEL_IS_AVAILABLE, IPC_CHANNEL_TRANSCRIBE } = require("../speech/speech-ipc.cjs");

const source = fs.readFileSync(path.join(__dirname, "preload.mts"), "utf8");

test("preload.mts's inlined IPC_CHANNEL_IS_AVAILABLE literal matches speech-ipc.cjs's own export", () => {
...
test("preload.mts's inlined IPC_CHANNEL_TRANSCRIBE literal matches speech-ipc.cjs's own export", () => {
