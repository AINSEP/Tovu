/**
 * @file The one holder of "which site dirs have a LIVE `tovu serve` right now" — `main.cjs`'s
 ...
 *
 * - `buildProjectRecord` (`project-ipc.cjs`) kept reporting `status: "running"` and the old port;
 ...
 * - `openSiteServer` (`main.cjs`) returned `already.server`, so "Start site" handed back the corpse
 ...
 * `project-registry.cjs`, `site-registry.cjs` and `project-delete-guard.cjs`.
 */
 ...
 *   right crash-safety row — `site-registry.cjs` can hold a live sibling instance's row for the
 *   same site dir, and closing by site dir alone would take that one too (D-07).
 *   Injected rather than reaching for `site-registry.cjs` directly so this module needs no
 *   `registryPath`, no filesystem, and no knowledge of what the caller does about it — `main.cjs`
...
module.exports = { createSiteSupervisor };
