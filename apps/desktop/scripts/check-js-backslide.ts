 ...
 * (`ADS-memory/reports/2026-09-12-desktop-typescript-migration-plan.md`) and why the one exception
 * (`src/speech/preload-speech.cjs`) is named exactly rather than matched by directory or pattern.
 *
...
/** The one file this migration could not convert — see `../src/js-backslide-guard.ts`'s header for
 *  why it is named exactly rather than exempted by directory or pattern. */
