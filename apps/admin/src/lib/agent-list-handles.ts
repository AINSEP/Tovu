/**
 * @file Shared decision for giving every item in a repeated list of rows/cards a distinct,
 * stable agent handle base — extracted from `features/settings/rules.ts`'s
 * `buildExternalMcpCardHandles` (that module's own doc comment has the fuller rationale) once a
 * second list screen needed the identical logic, per this workstream's "reuse or extract, never a
 * third copy" rule.
 *
 * The core problem: a `data-agent-element` handle is `[a-z0-9-]` only, but the ids driving a real
 * list (a form field's `id`, a content type's `key`, a redirect's numeric row id, ...) are
 * arbitrary strings. Two different ids can also slugify to the SAME handle. Both are resolved
 * here, once, so every list screen in this app derives its handles the same way instead of each
 * re-deriving (and possibly getting wrong) the same two rules.
 */
 ...
 * search then walks the full used-set for each collision); n is a list screen's row/card count,
 * which is operator-authored content and stays small in practice. Space O(n).
 */
