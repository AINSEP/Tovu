...
| file | what it is |
|---|---|
| `Appearance.tsx` | Theme/presentation settings (76 lines — a stub, not a placeholder for a bigger screen that never got built; see `panels.tsx`'s "Themes" nav label). |
| `index.ts` | The only surface `panels.tsx` may import. |

## Notes for anyone editing here

`Appearance.tsx` has **no unit test**. Treat a change here as unverified until you have driven it in
a browser.
