import type { AdminIdentityUser, SettingScope } from "../../lib/api";
...
import { usePrincipalSelector } from "./hooks/use-principal-selector.hooks";
import { useResetNamespaceDialog } from "./hooks/use-reset-namespace-dialog.hooks";
import { useSettings } from "./hooks/use-settings.hooks";
import { useSettingsContainer } from "./hooks/use-settings-container.hooks";
import { useValueEditor } from "./hooks/use-value-editor.hooks";

/**
 * @file Admin "Settings" screen (SPEC-007 `ui.spec.md`, tasks.md T045/T046) — markup only.
 *
 ...
 *
 * State, effects, and API calls live in `hooks/*.hooks.ts` — one hook per component that owns any
 * (`use-settings.hooks.ts` for `Settings`, `use-settings-container.hooks.ts` for
 * `SettingsContainer`, and one each for `PrincipalSelector`/`ValueEditor`/`ResetNamespaceDialog`).
 * Pure computation lives in `rules.ts`. What stays here is what actually renders: JSX structure,
 * static copy, and column/field layout.
 *
 ...
 *    accumulate into `NamespaceGroupList`'s `groups`. Implemented in
 *    `use-settings-container.hooks.ts`'s `loadNamespace`.
 *
 ...
 *    (which would misreport "not set"). Implemented in `rules.ts`'s
 *    `buildDetail`/`resolveSelectedDetail`/`layerStates`.
 *
 ...
 *    already held `settings.user.write`. Implemented in `Settings()` below and
 *    `rules.ts`'s `computeEditableScopes`.
 *
...
function PrincipalSelector({ usePrincipalSelectorHook = usePrincipalSelector, ...props }: PrincipalSelectorProps) {
  const { draft, setDraft, submit } = usePrincipalSelectorHook({
    value: props.value,
    onSubmitPrincipal: props.onSubmitPrincipal,
    onClearPrincipal: props.onClearPrincipal,
  });
...
interface ValueEditorProps {
  ...
  onClearValue: () => void;
  /** Dependency injection seam for tests — same convention as `PostsProps.usePostsHook`. */
  useValueEditorHook?: typeof useValueEditor;
}

function ValueEditor({ useValueEditorHook = useValueEditor, ...props }: ValueEditorProps) {
  const { draft, setDraft, validationError, setValidationError, submit } = useValueEditorHook({
...
interface ResetNamespaceDialogProps {
  namespace: string;
  scope: SettingScope;
  onConfirmReset: () => void;
  onCancel: () => void;
  /** Dependency injection seam for tests — same convention as `PostsProps.usePostsHook`. */
  useResetNamespaceDialogHook?: typeof useResetNamespaceDialog;
}

function ResetNamespaceDialog({ useResetNamespaceDialogHook = useResetNamespaceDialog, ...props }: ResetNamespaceDialogProps) {
  useResetNamespaceDialogHook({ onCancel: props.onCancel });
...
function SettingDetailPanel(props: {
  ...
  const { detail } = props;
  const states = layerStates(detail);
  ...
  return (
    <div className="settings-detail-panel">
      ...
      <div className="settings-layer-grid">
        ...
        {layerCell({ label: "User", value: detail.user, state: states.user })}
        {layerCell({ label: "Workspace", value: detail.workspace, state: states.workspace })}
        {layerCell({ label: "Global", value: detail.global, state: states.global })}
        {layerCell({ label: "Default", value: detail.default, state: states.default })}
      </div>
...
interface SettingsContainerProps {
  ...
  /** Dependency injection seam for tests — same convention as `PostsProps.usePostsHook`. */
  useSettingsContainerHook?: typeof useSettingsContainer;
}

function SettingsContainer({ useSettingsContainerHook = useSettingsContainer, ...props }: SettingsContainerProps) {
  const c = useSettingsContainerHook({
    canWriteScopes: props.canWriteScopes,
    selfPrincipalId: props.selfPrincipalId,
    users: props.users,
  });
  ...
  return (
    <div className="settings-screen">
      ...
      <form className="notice settings-namespace-loader" onSubmit={c.onLoadNamespace}>
        <label htmlFor="settings-namespace-input">Namespace</label>
        <span className="editor-actions">
          <input
            id="settings-namespace-input"
            value={c.namespaceInput}
            onChange={(e) => c.setNamespaceInput(e.target.value)}
            placeholder="e.g. core.presentation"
          />
          <button type="submit" disabled={c.isLoading}>
            {c.isLoading ? "Loading…" : "Load"}
          </button>
        </span>
      ...
      <PrincipalSelector
        ...
        value={c.principalValue}
        validationState={c.principalValidationState}
        lastError={c.principalLastError}
        onSubmitPrincipal={c.onSubmitPrincipal}
        onClearPrincipal={c.onClearPrincipal}
      />

      {c.error ? <ErrorBanner message={c.error} onRetry={c.onRetryLoad} /> : null}

      <div className="settings-body">
        {c.groups.length === 0 && !c.isLoading ? (
          ...
          <NamespaceGroupList
            ...
            onSelectSetting={c.onSelectSetting}
          />
        )}

        {c.sel && c.detail ? (
          <SettingDetailPanel
            namespace={c.sel.namespace}
            settingKey={c.sel.key}
            detail={c.detail}
            editableScopes={c.editableScopes}
            canReset={props.canReset}
            saving={c.saving}
            onSubmitValue={c.onSubmitValue}
            onClearValue={c.onClearValue}
            onRequestReset={c.onRequestReset}
          />
        ) : null}
      </div>
      ...
      {c.pendingReset ? (
        <ResetNamespaceDialog
          namespace={c.pendingReset.namespace}
          scope={c.pendingReset.scope}
          onConfirmReset={c.onConfirmReset}
          onCancel={c.onCancelReset}
        />
      ) : null}
    </div>
  );
...
export function Settings({ useSettingsHook = useSettings }: SettingsProps = {}) {
  const { loading, error, selfPrincipalId, permissions, users } = useSettingsHook();
