...
/**
 * @file `rules.ts`'s HTML-attributes allowlist — the admin-side half of the security control for
 * the media edit form's `HTML attributes` field (`Media.tsx`'s `MediaHtmlAttributesField`). Media
 * metadata is authored here but rendered on the public site, so this is a stored-XSS boundary: an
 * `on*` handler, a `javascript:` value, or any name not on the allowlist must be rejected outright,
 * with a specific, visible reason naming the rejected attribute — never silently sanitized, never a
