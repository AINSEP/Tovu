...
import type { ContentTransportBundleRepoPort, StagedBundleRecord } from "#src/features/content-transport/bundle-staging";
import { contentTransportBundles } from "../schema.js";
 ...
 *
 * Real SQLite `ContentTransportBundleRepoPort` adapter over `content_transport_bundles`
...
type Row = typeof contentTransportBundles.$inferSelect;
...
export class SqliteContentTransportBundleRepo implements ContentTransportBundleRepoPort {
  ...
  async save(record: StagedBundleRecord): Promise<void> {
    this.db.insert(contentTransportBundles).values({ ...record }).run();
  }

  async findById(input: { workspaceId: string; id: string }): Promise<StagedBundleRecord | null> {
    const row = this.db
      .select()
      .from(contentTransportBundles)
      .where(and(eq(contentTransportBundles.workspaceId, input.workspaceId), eq(contentTransportBundles.id, input.id)))
      .get();
