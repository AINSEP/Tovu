/**
 * @file Proves `src/platform/db/schema.postgres.ts` still matches what `src/platform/db/schema.ts` generates.
 *
...
test("schema.postgres.ts is up to date with schema.ts (run the generator and commit if this fails)", () => {
  ...
  assert.doesNotThrow(run, "src/platform/db/schema.postgres.ts has drifted from src/platform/db/schema.ts");
