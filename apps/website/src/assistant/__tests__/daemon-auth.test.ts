...
// ---------------------------------------------------------------------------------------------
// deriveDelegatedToolCredential / requireDelegatedToolCredential — the per-run credential fix.
//
// `agent-daemon-server.ts` mounts `requireDelegatedToolCredential()` at `DELEGATED_TOOL_CALLS_PATH`
// in this exact sequence: `requireAgentDaemonToken({ exemptPaths: [DELEGATED_TOOL_CALLS_PATH] })`,
// then `express.json()`, then this gate, then `registerDelegatedToolRoutes`. `bootDelegatedToolServer`
// reproduces that real composition so a rejection here proves the SHIPPED middleware chain rejects
// it, not a check written in this test file.
// ---------------------------------------------------------------------------------------------

/** Mounts the real production gate sequence for `DELEGATED_TOOL_CALLS_PATH`, ending in a probe. */
async function bootDelegatedToolServer(env: NodeJS.ProcessEnv) {
  const app = express();
  let probeReached = 0;
  app.use(requireAgentDaemonToken({ env, exemptPaths: [DELEGATED_TOOL_CALLS_PATH] }));
  app.use(express.json());
  app.use(DELEGATED_TOOL_CALLS_PATH, requireDelegatedToolCredential({ env }));
  app.post(DELEGATED_TOOL_CALLS_PATH, (_req, res) => {
    probeReached += 1;
    res.status(201).json({ started: true });
  });
...
test("deriveDelegatedToolCredential returns a deterministic 64-char hex HMAC, distinct per secret and per runId", () => {
  const a = deriveDelegatedToolCredential(TOKEN, "run-1");
  const b = deriveDelegatedToolCredential(TOKEN, "run-1");
  const differentRun = deriveDelegatedToolCredential(TOKEN, "run-2");
  const differentSecret = deriveDelegatedToolCredential("b".repeat(64), "run-1");

  assert.match(a, /^[0-9a-f]{64}$/, "a SHA-256 HMAC digest, hex-encoded");
  assert.equal(a, b, "deterministic — same secret and runId must always derive the same credential");
  assert.notEqual(a, differentRun, "a credential derived for one run must not match another run's");
  assert.notEqual(a, differentSecret, "a credential derived under one boot secret must not match another's");
  assert.notEqual(a, TOKEN, "the derived credential must never equal the raw boot secret itself — the whole point of this fix");
});

test("requireDelegatedToolCredential: the correct per-run credential is accepted", async (t) => {
  const harness = await bootDelegatedToolServer({ [AGENT_DAEMON_TOKEN_ENV_VAR]: TOKEN });
  t.after(harness.close);

  const credential = deriveDelegatedToolCredential(TOKEN, "run-1");
  const res = await fetch(`${harness.baseUrl}${DELEGATED_TOOL_CALLS_PATH}`, {
    method: "POST",
...
test("requireDelegatedToolCredential: the raw boot-wide token is now REJECTED — that is the whole point of this fix", async (t) => {
  const harness = await bootDelegatedToolServer({ [AGENT_DAEMON_TOKEN_ENV_VAR]: TOKEN });
  t.after(harness.close);

  // Before this fix, `mcp-injection.ts` handed a spawned CLI's `jini-mcp` subprocess this exact
  // raw token. If it (or anything it can reach) still worked as a delegated-tool-calls credential,
  // the fix bought nothing.
  const res = await fetch(`${harness.baseUrl}${DELEGATED_TOOL_CALLS_PATH}`, {
    method: "POST",
    headers: { authorization: `Bearer ${TOKEN}`, "content-type": "application/json" },
    body: JSON.stringify({ runId: "run-1", toolId: "collections_content_type_define" }),
...
test("requireDelegatedToolCredential: a credential derived for a DIFFERENT run is rejected", async (t) => {
  const harness = await bootDelegatedToolServer({ [AGENT_DAEMON_TOKEN_ENV_VAR]: TOKEN });
  t.after(harness.close);

  const wrongRunCredential = deriveDelegatedToolCredential(TOKEN, "run-2");
  const res = await fetch(`${harness.baseUrl}${DELEGATED_TOOL_CALLS_PATH}`, {
    method: "POST",
    headers: { authorization: `Bearer ${wrongRunCredential}`, "content-type": "application/json" },
    body: JSON.stringify({ runId: "run-1", toolId: "collections_content_type_define" }),
  });

  assert.equal(res.status, 401, "run-2's credential must not authorize a call claiming to be run-1");
  assert.equal(harness.probeCount(), 0);
});

test("requireDelegatedToolCredential: 401s a request with no Authorization header at all", async (t) => {
  const harness = await bootDelegatedToolServer({ [AGENT_DAEMON_TOKEN_ENV_VAR]: TOKEN });
  t.after(harness.close);
...
test("requireDelegatedToolCredential: fails closed with 503 when the token env var is unset", async (t) => {
  const harness = await bootDelegatedToolServer({});
  t.after(harness.close);
...
test("requireDelegatedToolCredential: a body with no usable runId is waved through unauthenticated, on purpose — parseDelegatedToolExecute's own validation error is what actually refuses it downstream", async (t) => {
  const harness = await bootDelegatedToolServer({ [AGENT_DAEMON_TOKEN_ENV_VAR]: TOKEN });
  t.after(harness.close);
  ...
  const res = await fetch(`${harness.baseUrl}${DELEGATED_TOOL_CALLS_PATH}`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ toolId: "collections_content_type_define" }),
  });

  assert.equal(res.status, 201, "no runId means this gate has nothing to check a credential against yet");
  assert.equal(harness.probeCount(), 1);
});
