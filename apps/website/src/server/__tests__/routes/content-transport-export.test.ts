...
} from "#src/features/content-transport/type-registry";
...
import type { ContentTransportContributor, PackedEntity } from "#src/features/content-transport/type-registry";
 ...
 *
 * Route-level tests for `GET /content-transport/export`, against the hermetic in-memory composition
 ...
 * against a REAL SQLite db, in `../integration/content-transport-export-no-leak.integration.test.ts`
 ...
 * mount-path guard, the two-permission-check shape (route-level `content.transport.read` vs.
 ...
 *  built) `content.transport.read` builtin-role grant. */
...
async function fetchExport(baseUrl: string, cookie: string): Promise<{ status: number; raw: string }> {
  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/export`, { headers: { cookie } });
  return { status: res.status, raw: await res.text() };
}

test("GET .../content-transport/export 404s when the URL workspace does not match this composition's own workspace", async (t) => {
  const { server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));

  const cookie = await loginAsOwner(baseUrl);
  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/some-other-workspace/content-transport/export`, { headers: { cookie } });
  assert.equal(res.status, 404);
});

test("GET .../content-transport/export is 401 without a credential and 403 for an authenticated principal lacking content.transport.read", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));

  const anonymous = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/export`);
...
test("GET .../content-transport/export streams registered post/page entities for a principal holding content.transport.read and content.write", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  ...
  const cookie = await loginWithPermissions(deps, baseUrl, {
    ...
    permissions: ["content.transport.read", "content.write"],
  });
...
test("GET .../content-transport/export omits a type the principal lacks — 200 with entities: [], never a 500", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  ...
  // page's declared `permission` — see `features/post/content-transport.ts`'s `buildHandler`).
  const cookie = await loginWithPermissions(deps, baseUrl, {
    username: "read-only-transport",
    permissions: ["content.transport.read"],
  });
  const { status, raw } = await fetchExport(baseUrl, cookie);
  assert.equal(status, 200, raw);
  const bundle = JSON.parse(raw) as ExportBundle;
...
test("GET .../content-transport/export is an ALLOWLIST: an empty contributor registry yields an empty bundle even though real posts exist", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  t.after(() => installFirstPartyTransportTypes()); // restore for any later test in this process
...
test("GET .../content-transport/export reads the contributor registry FRESH per request — a type registered after the first call is present on the very next one", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  t.after(() => installFirstPartyTransportTypes()); // restore the real registry for any later test
  ...
  const canaryContributor: ContentTransportContributor = {
    entityType: "canary",
    dependsOn: [],
    build: () => ({
      entityType: "canary",
      permission: "content.transport.read", // trivially satisfied by the caller in this test
      dependsOn: [],
      async *pack() {
        yield canaryEntity;
      },
  ...
  registerContentTransportContributor(canaryContributor);
