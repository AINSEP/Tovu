 ...
 * `content-transport-export.test.ts`'s own harness (same file, same login helpers duplicated here
 ...
 *  Task 9's `content.transport.apply` builtin-role grant has landed yet. */
...
test("PUT .../blobs/:sha 404s when the URL workspace does not match this composition's own workspace", async (t) => {
  ...
  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/some-other-workspace/content-transport/blobs/${sha256Of("hello")}`, {
    method: "PUT",
...
test("PUT .../blobs/:sha is 401 without a credential and 403 for a principal lacking content.transport.apply", async (t) => {
  const { server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));

  const anonymous = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/${"a".repeat(64)}`, {
    method: "PUT",
    headers: { "content-type": "application/json" },
...
test("PUT .../blobs/:sha writes real bytes once, then reports written:false on re-upload of the same sha (idempotent dedupe)", async (t) => {
  ...
  const first = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/${sha}`, {
    method: "PUT",
    headers: { "content-type": "application/json", cookie },
  ...
  const second = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/${sha}`, {
    method: "PUT",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({ dataBase64 }),
  });
...
test("PUT .../blobs/:sha REFUSES bytes that do not hash to the claimed sha256 (security property, plan §4 task 6)", async (t) => {
  ...
  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/${realSha}`, {
    method: "PUT",
    headers: { "content-type": "application/json", cookie },
  ...
  const probe = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/probe`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
...
test("PUT .../blobs/:sha 400s on a malformed sha param (not lowercase 64-hex)", async (t) => {
  ...
  const cookie = await loginAsOwner(baseUrl);
  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/not-a-sha`, {
    method: "PUT",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({ dataBase64: "aGVsbG8=" }),
  });
...
test("POST .../blobs/probe SHORT-CIRCUITS an already-present sha (required Task 6 property)", async (t) => {
  ...
  const upload = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/${presentSha}`, {
    method: "PUT",
    headers: { "content-type": "application/json", cookie },
...
test("POST .../blobs/probe 400s on a non-array / oversized / malformed shas payload", async (t) => {
  ...
  const notArray = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/probe`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
  ...
  const malformed = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/blobs/probe`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
