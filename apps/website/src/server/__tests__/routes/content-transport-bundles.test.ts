...
import { loadActiveBundle } from "#src/features/content-transport/bundle-staging";
 ...
 *
 * Route-level tests for `POST .../content-transport/bundles`, against the hermetic in-memory
 * composition root, mirroring `content-transport-export.test.ts`'s own harness.
 *
 * The "a bundle past its `expiresAt` is refused" property is proved directly, at the unit level,
 * against `loadActiveBundle` in `features/content-transport/__tests__/bundle-staging.test.ts` — this
 * file re-proves it end-to-end through the real HTTP route + real `deps.contentTransportBundleRepo`
...
test("POST .../content-transport/bundles 404s when the URL workspace does not match this composition's own workspace", async (t) => {
  const { server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));

  const cookie = await loginAsOwner(baseUrl);
  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/some-other-workspace/content-transport/bundles`, {
    method: "POST",
...
test("POST .../content-transport/bundles is 401 without a credential", async (t) => {
  const { server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));

  const res = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/bundles`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(validBundleBody()),
  });
...
test("POST .../content-transport/bundles stages a bundle and returns {bundleId, expiresAt} in the future", async (t) => {
  const { server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
...
test("POST .../content-transport/bundles rejects a malformed body (wrong shape for hashVersion/entities/blobManifest)", async (t) => {
  const { server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));

  const cookie = await loginAsOwner(baseUrl);

  const badHashVersion = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/bundles`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
  ...
  const badEntities = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/bundles`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
  ...
  const badBlobManifest = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/bundles`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
...
test("a staged bundle is retrievable via the real repo wiring, and ignores a caller-supplied expiresAt-like field", async (t) => {
  ...
  const active = await loadActiveBundle({
    repo: deps.contentTransportBundleRepo,
