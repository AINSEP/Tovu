 ...
 *
 * Real-HTTP integration tests for the `content.transport` gated import ceremony
 * (`import/{plan,confirm,execute}`), mirroring `taxonomy-merge-term-routes.test.ts`'s and
 * `content-transport-bundles.test.ts`'s own harness. The "agent principal cannot confirm" property
...
async function stagePostBundle(baseUrl: string, cookie: string, title: string): Promise<{ bundleId: string; postId: string }> {
  ...
  const exportRes = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/export`, { headers: { cookie } });
  const bundle = await expectJson<Record<string, unknown>>(exportRes, 200);

  const stageRes = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/bundles`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
...
async function planAndConfirm(baseUrl: string, cookie: string, bundleId: string): Promise<string> {
  const planRes = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/import/plan`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
  ...
  const confirmRes = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/import/confirm`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
...
test("content-transport import: plan -> confirm -> execute reaches the Task 8 seam and captures a restore point first", async (t) => {
  ...
  deps.contentTransportApplyPort = {
  ...
  const executeRes = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/import/execute`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
...
test("content-transport import: a destination edit between confirm and execute is rejected PLAN_STALE, never silently applied", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  const cookie = await loginAsOwner(baseUrl);
  deps.contentTransportApplyPort = { applyReport: async () => ({ changeSetIds: [] }) };
...
test("content-transport import: replaying an already-redeemed confirmation token is rejected TOKEN_ALREADY_REDEEMED", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  const cookie = await loginAsOwner(baseUrl);
  deps.contentTransportApplyPort = { applyReport: async () => ({ changeSetIds: [] }) };
  ...
  // succeeds or throws).
  await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/import/execute`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({ bundleId, confirmationToken }),
  });

  const replayRes = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/import/execute`, {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({ bundleId, confirmationToken }),
  });
...
test("content-transport import: execute refuses RESTORE_POINT_UNAVAILABLE with no override when the site's restore-point mechanism is unavailable", async (t) => {
  const { deps, server, baseUrl } = await startServer();
  t.after(() => new Promise<void>((resolve) => server.close(() => resolve())));
  const cookie = await loginAsOwner(baseUrl);
...
test("gated-mutations gateway: an agent principal can never confirm a content-transport import plan", async () => {
