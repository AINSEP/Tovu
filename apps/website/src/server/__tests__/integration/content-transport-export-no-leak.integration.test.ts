 ...
 * content-transport types, never a deny-list — so a table nobody thought to exclude cannot leak
 * "by omission". `content-transport-export.test.ts` proves the MECHANISM (registry read fresh,
 ...
 * `member*`), `form_submissions`, and all four `content_transport_*` tables themselves (plan §5 risk
 ...
 *  `content_transport_peers`) need a real workspace row first — inserted here for exactly that
...
function plantCanaries(db: ReturnType<typeof openContentDb>): void {
  ...
  db.insert(schema.contentTransportBaselines)
    .values({
      workspaceId: WORKSPACE,
      peerPrincipalId: "canary-principal",
      entityType: "post",
      entityId: "canary-baseline-entity",
      hashAtLastSync: marker("content_transport_baselines"),
      hashVersion: 1,
      syncedAt: NOW,
      runId: "canary-run",
    })
  ...
  db.insert(schema.contentTransportRuns)
    .values({
      id: "canary-run",
      workspaceId: WORKSPACE,
      direction: "export",
      ...
      actorId: "canary-principal",
      startedAt: NOW,
      reportJson: JSON.stringify({ marker: marker("content_transport_runs") }),
    })
    .run();

  db.insert(schema.contentTransportBundles)
    .values({
      id: "canary-bundle",
      workspaceId: WORKSPACE,
      sourcePrincipalId: "canary-principal",
      hashVersion: 1,
      entitiesJson: JSON.stringify({ marker: marker("content_transport_bundles") }),
      blobManifestJson: "[]",
      sizeBytes: 0,
      receivedAt: NOW,
      expiresAt: NOW,
  ...
  db.insert(schema.contentTransportPeers)
    .values({
      id: "canary-peer",
      workspaceId: WORKSPACE,
      label: marker("content_transport_peers"),
...
test("GET .../content-transport/export never leaks a row from any sensitive table, even for the most-privileged (wildcard) caller", async (t) => {
  ...
  const exported = await fetch(`${baseUrl}/api/admin/v1/workspaces/${WORKSPACE}/content-transport/export`, {
    headers: { cookie },
  });
  ...
  for (const table of SENSITIVE_TABLES) {
    assert.ok(!raw.includes(marker(table)), `sensitive table '${table}' leaked into the content-transport export bundle`);
  }
});
