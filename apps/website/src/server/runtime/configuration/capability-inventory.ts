...
export const CAPABILITY_INVENTORY: readonly CapabilityInventoryEntry[] = [
  ...
  {
    name: "database",
    ownerModule: "features/database",
    classification: "production",
    sourceOfTruth: "sqlite (sidecar ops/database-journal.db, SqliteDatabaseLedgerRepo/SqliteRestorePointsRepo)",
    readinessDependencies: ["ops/database-journal.db sidecar connection"],
    startupCriticality: "critical",
    securityDependencies: ["identity authorize() gate"],
    restartTestOwner: "features/database test suite",
    hasDurableAdapter: true,
    sourceHints: ["SqliteDatabaseLedgerRepo", "databaseLedgerRepo"],
  },
  {
    name: "content-types",
  ...
  },
  {
    ...
    classification: "production",
    sourceOfTruth: "sqlite (content.db, SqliteTokenStore — SPEC-022 durability fix, gated_mutation_tokens table, migration 0052)",
    readinessDependencies: ["content.db connection"],
    startupCriticality: "critical",
    ...
    ownerAmbiguous: true,
    ambiguityNote:
      "Owned by core/gated-mutations as the port/gateway definition, but configured and instantiated per composition root (server/deps.ts and server/app.ts each call buildGatewayDeps independently) — no single feature module is the sole owner of its runtime configuration. server/deps.ts (the real production root) now passes a durable SqliteTokenStore; server/app.ts's hermetic in-memory composition still gets the default InMemoryTokenStore, correctly (it is in-memory everywhere by design, per this file's own header).",
    sourceHints: ["gatedMutations", "buildGatewayDeps", "SqliteTokenStore"],
  },
  {
    name: "outbox",
  ...
  },
  {
    ...
    ownerModule: "origin",
    classification: "production",
    sourceOfTruth: "sqlite (content.db, SqliteOriginSettingRepo — ADR-046 Phase 1, 2026-07-16); registered from TOVU_PUBLIC_URL via registerConfiguredOrigin (2026-09-18), falling back to the hardcoded dev-capability localhost seed in local mode only",
    readinessDependencies: ["TOVU_PUBLIC_URL set to this deployment's public https origin — a production boot without it registers no origin at all and every public URL (sitemap loc, canonical, og:url, newsletter/magic links) degrades to a relative path", "reachability/ownership verification of the configured origin — ADR-040's own v0.1 open item; registration authority today is process-environment access, and no admin route exists"],
    startupCriticality: "critical",
