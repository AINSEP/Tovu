...
import { registerAdminNewsletterCancelCampaignRoute } from "../cancel-campaign.js";
...
function buildApp(depsOverrides: Partial<NewsletterRouteDeps> = {}): {
  ...
  registerAdminNewsletterCancelCampaignRoute(app, deps);
  return { app, deps };
}
...
test("cancel-campaign: mismatched workspaceId 404s", async (t) => {
  const { app } = buildApp();
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/other-ws/newsletter/campaigns/camp-1/cancel`
  );
  assert.equal(status, 404);
  assert.deepEqual(json, { error: "workspace was not found" });
});

test("cancel-campaign: forbidden 403s when unauthorized", async (t) => {
  const { app } = buildApp({
  ...
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/${WORKSPACE_ID}/newsletter/campaigns/camp-1/cancel`
  );
...
test("cancel-campaign: successful cancel transitions draft campaign to canceled", async (t) => {
...
test("cancel-campaign: not found campaign returns 404", async (t) => {
  const { app } = buildApp();
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/${WORKSPACE_ID}/newsletter/campaigns/non-existent/cancel`
  );
  assert.equal(status, 404);
  assert.equal((json as { code?: string }).code, "NEWSLETTER_CAMPAIGN_NOT_FOUND");
});

test("cancel-campaign: invalid state transition returns 409 conflict", async (t) => {
  const { app, deps } = buildApp();
  const campaign = makeCampaign({ id: "sent-camp", status: "sent" });
  await deps.newsletterCampaignRepo.saveCampaignRow(campaign);
...
test("cancel-campaign: generic unexpected error maps to 500", async (t) => {
  const { app, deps } = buildApp();
...
test("cancel-campaign: undefined workspaceId fallback via direct handler invocation", async () => {
  ...
  const handler = extractRouteHandler(
    app,
    "post",
    "/api/admin/v1/workspaces/:workspaceId/newsletter/campaigns/:id/cancel"
  );
