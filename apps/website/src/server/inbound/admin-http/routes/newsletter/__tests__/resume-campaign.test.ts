...
function buildApp(depsOverrides: Partial<NewsletterRouteDeps> = {}): {
  ...
  registerAdminNewsletterResumeCampaignRoute(app, deps);
  return { app, deps };
}
...
test("resume-campaign: mismatched workspaceId 404s", async (t) => {
  const { app } = buildApp();
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/other-ws/newsletter/campaigns/camp-1/resume`
  );
  assert.equal(status, 404);
  assert.deepEqual(json, { error: "workspace was not found" });
});

test("resume-campaign: forbidden 403s when unauthorized", async (t) => {
  const { app } = buildApp({
  ...
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/${WORKSPACE_ID}/newsletter/campaigns/camp-1/resume`
  );
...
test("resume-campaign: successful resume transitions paused campaign to sending", async (t) => {
  ...
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/${WORKSPACE_ID}/newsletter/campaigns/${campaign.id}/resume`
  );
  assert.equal(status, 200);
  const body = json as { data?: CampaignRecord };
  assert.equal(body.data?.id, campaign.id);
  assert.equal(body.data?.status, "sending");
...
test("resume-campaign: not found campaign returns 404", async (t) => {
  const { app } = buildApp();
  const { status, json } = await post(
    t,
    app,
    `/api/admin/v1/workspaces/${WORKSPACE_ID}/newsletter/campaigns/non-existent/resume`
  );
  assert.equal(status, 404);
  assert.equal((json as { code?: string }).code, "NEWSLETTER_CAMPAIGN_NOT_FOUND");
});

test("resume-campaign: invalid state transition returns 409 conflict", async (t) => {
  const { app, deps } = buildApp();
...
test("resume-campaign: generic unexpected error maps to 500", async (t) => {
  const { app, deps } = buildApp();
...
test("resume-campaign: undefined workspaceId fallback via direct handler invocation", async () => {
  const { app } = buildApp();
  const handler = extractRouteHandler(
    app,
    "post",
    "/api/admin/v1/workspaces/:workspaceId/newsletter/campaigns/:id/resume"
  );
