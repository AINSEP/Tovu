import { stageBundle, CONTENT_TRANSPORT_BUNDLE_MAX_BODY_BYTES } from "#src/features/content-transport/bundle-staging";
 ...
 *
 * `POST /api/admin/v1/workspaces/:workspaceId/content-transport/bundles` — stages a received bundle
 * (Task 4's export envelope: `{hashVersion, sourceLabel, entities, blobManifest}`, whether pushed by
 * a peer or fetched by this instance's own pull) in `content_transport_bundles`, so Task 7's gated
...
export const registerContentTransportBundleCreateRoute: ContentTransportRouteRegistrar = (app, deps) => {
  app.post(
    "/api/admin/v1/workspaces/:workspaceId/content-transport/bundles",
    rejectOversizedJsonBody({ maxBytes: CONTENT_TRANSPORT_BUNDLE_MAX_BODY_BYTES }),
    async (req, res) => {
      ...
      try {
        ...
        const { bundleId, expiresAt } = await stageBundle(
          ...
          { repo: deps.contentTransportBundleRepo, clock: deps.clock, idGen: deps.idGen }
        );
