...
import {
  buildContentTransportImportHooks,
  ContentTransportApplyNotImplementedError,
  ContentTransportBundleNotFoundError,
} from "#src/features/content-transport/gated-hooks";
import { executeContentTransportImport, RestorePointUnavailableError } from "#src/features/content-transport/execute-import";
 ...
 *
 * `POST /api/admin/v1/workspaces/:workspaceId/content-transport/import/{plan,confirm,execute}` —
 * the gated import ceremony over a bundle already staged by Task 6's
 * `POST .../content-transport/bundles`. Mirrors `taxonomy/merge-term.ts`'s 3-endpoint-in-one-file
 ...
 * steps, since `buildContentTransportImportHooks` closes over it fresh per request (mirrors
...
function statusFor(err: unknown): { status: number; code: string } {
  ...
  if (err instanceof ContentTransportBundleNotFoundError) return { status: 404, code: "BUNDLE_NOT_FOUND" };
  ...
  if (err instanceof ContentTransportApplyNotImplementedError) return { status: 501, code: "APPLY_NOT_IMPLEMENTED" };
  return { status: 500, code: "INTERNAL_ERROR" };
}

/** Builds the narrow `ContentTransportDeps` bag every registered contributor's `build()` closes
 *  over — same shape as `export.ts`'s own `toContentTransportDeps` (duplicated locally rather than
...
export const registerContentTransportImportRoutes: ContentTransportRouteRegistrar = (app: Express, deps) => {
  function buildHooks(bundleId: string, actorId: string) {
    return buildContentTransportImportHooks({
      workspaceId: deps.workspaceId,
      bundleId,
      actorId,
      clock: deps.clock,
      idGen: deps.idGen,
      contentTransportDeps: toContentTransportDeps(deps),
      bundleRepo: deps.contentTransportBundleRepo,
      baselineRepo: deps.contentTransportBaselineRepo,
      blobStore: deps.blobStore,
      dbOps: deps.dbOps,
      restorePointsRepo: deps.restorePointsRepo,
      applyPort: deps.contentTransportApplyPort,
    });
  }

  app.post("/api/admin/v1/workspaces/:workspaceId/content-transport/import/plan", async (req, res) => {
  ...
  app.post("/api/admin/v1/workspaces/:workspaceId/content-transport/import/confirm", async (req, res) => {
    if (String(req.params.workspaceId ?? "") !== deps.workspaceId) {
      res.status(404).json({ error: "workspace was not found" });
      return;
    }
  ...
  app.post("/api/admin/v1/workspaces/:workspaceId/content-transport/import/execute", async (req, res) => {
    if (String(req.params.workspaceId ?? "") !== deps.workspaceId) {
      res.status(404).json({ error: "workspace was not found" });
      return;
    }
    try {
      ...
      const preCheck = await authorizeForHooks(
        ...
        { principalId: principal.id, permission: "content.transport.apply" }
      );
      if (!preCheck.allowed) {
        res.status(403).json({
          error: `principal '${principal.id}' is not authorized for 'content.transport.apply' (${preCheck.reason})`,
          code: "NOT_AUTHORIZED",
        });
        return;
      }
      ...
      const result = await executeContentTransportImport({
        workspaceId: deps.workspaceId,
