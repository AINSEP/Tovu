...
export async function planGitHubFileWrite(deps: GitHubWriteFilesDeps, input: PlanGitHubFileWriteInput): Promise<GitHubWriteFilesPlanResult> {
  ...
  const refResult = await githubSend(deps, input.connection, { method: "GET", url: `${repoPath}/git/ref/heads/${encPath(input.branch)}` });
  if (refResult.kind !== "response") return nonResponseFailure(refResult);
  if (refResult.response.status === 404) {
    return { ok: false, code: "branch-not-found", message: `branch '${input.branch}' does not exist in ${input.owner}/${input.repo}` };
  }
  if (!isOk(refResult.response)) return { ok: false, code: "provider-error", message: await providerErrorMessage(refResult.response, "GitHub branch lookup failed") };
  const refBody = parseJsonBody(refResult.response);
  if (!refBody.ok) return { ok: false, code: "provider-error", message: refBody.message };
  const object = refBody.json.object as Record<string, unknown> | undefined;
  const parentCommitSha = typeof object?.sha === "string" ? object.sha : "";
  if (!parentCommitSha) return { ok: false, code: "provider-error", message: "GitHub branch lookup response did not include a sha" };

  const commitResult = await githubSend(deps, input.connection, { method: "GET", url: `${repoPath}/git/commits/${enc(parentCommitSha)}` });
  if (commitResult.kind !== "response") return nonResponseFailure(commitResult);
  if (!isOk(commitResult.response)) return { ok: false, code: "provider-error", message: await providerErrorMessage(commitResult.response, "GitHub parent-commit lookup failed") };
  const commitBody = parseJsonBody(commitResult.response);
  if (!commitBody.ok) return { ok: false, code: "provider-error", message: commitBody.message };
  const tree = commitBody.json.tree as Record<string, unknown> | undefined;
  const baseTreeSha = typeof tree?.sha === "string" ? tree.sha : "";
  ...
  for (const file of input.files) {
    const existsResult = await githubSend(deps, input.connection, {
      method: "GET",
      url: `${repoPath}/contents/${encPath(file.path)}?ref=${enc(input.branch)}`,
    });
    if (existsResult.kind !== "response") return nonResponseFailure(existsResult);
    if (existsResult.response.status === 404) {
      fileStates.push({ path: file.path, exists: false });
      continue;
    }
    if (!isOk(existsResult.response)) {
      return { ok: false, code: "provider-error", message: await providerErrorMessage(existsResult.response, `GitHub lookup of '${file.path}' failed`) };
    }
    fileStates.push({ path: file.path, exists: true });
  }

  return { ok: true, plan: { parentCommitSha, baseTreeSha, fileStates } };
}
...
async function createBlob(deps: GitHubWriteFilesDeps, connection: CustomProviderConnectionInput, repoPath: string, content: string): Promise<{ ok: true; sha: string } | GitHubWriteFilesFailure> {
  const result = await githubSend(deps, connection, {
    method: "POST",
  ...
  if (!isOk(result.response)) return { ok: false, code: "provider-error", message: await providerErrorMessage(result.response, "GitHub blob creation failed") };
...
/**
 * The write itself, run only after a human has confirmed: one blob per distinct file content
 * (deduped by a local sha256 key, matching `github-git-provider.ts`'s own optimization — a dedup key
 * only, not a wire-protocol requirement), one tree built ON TOP of `plan.baseTreeSha`, one commit
 * object, and one non-force ref update. `plan` must come from a prior {@link planGitHubFileWrite}
 * call against the SAME `files` list — this function trusts `plan.baseTreeSha`/`parentCommitSha`
 * rather than re-fetching them, so the confirmation dialog and the actual write are guaranteed to
 * build on the identical tree the human was shown.
 *
...
export async function commitGitHubFiles(deps: GitHubWriteFilesDeps, input: CommitGitHubFilesInput, plan: GitHubWriteFilesPlan): Promise<GitHubCommitFilesResult> {
  ...
  for (const file of input.files) {
    ...
    if (!blobSha) {
      const blobResult = await createBlob(deps, input.connection, repoPath, file.content);
  ...
  const treeResult = await githubSend(deps, input.connection, {
    method: "POST",
    url: `${repoPath}/git/trees`,
    // `base_tree` is ALWAYS sent — see this file's header, "KEPT: base_tree threading" — every path
    // not named in `treeEntries` is inherited unchanged from the branch's current tree.
    body: JSON.stringify({ tree: treeEntries, base_tree: plan.baseTreeSha }),
  });
  if (treeResult.kind !== "response") return nonResponseFailure(treeResult);
  if (!isOk(treeResult.response)) return { ok: false, code: "provider-error", message: await providerErrorMessage(treeResult.response, "GitHub tree creation failed") };
  const treeBody = parseJsonBody(treeResult.response);
  if (!treeBody.ok) return { ok: false, code: "provider-error", message: treeBody.message };
  const newTreeSha = typeof treeBody.json.sha === "string" ? treeBody.json.sha : "";
  if (!newTreeSha) return { ok: false, code: "provider-error", message: "GitHub tree creation response did not include a sha" };

  const commitResult = await githubSend(deps, input.connection, {
    method: "POST",
    url: `${repoPath}/git/commits`,
    body: JSON.stringify({ message: input.commitMessage, tree: newTreeSha, parents: [plan.parentCommitSha] }),
  });
  if (commitResult.kind !== "response") return nonResponseFailure(commitResult);
  if (!isOk(commitResult.response)) return { ok: false, code: "provider-error", message: await providerErrorMessage(commitResult.response, "GitHub commit creation failed") };
  const commitBody = parseJsonBody(commitResult.response);
  if (!commitBody.ok) return { ok: false, code: "provider-error", message: commitBody.message };
  const commitSha = typeof commitBody.json.sha === "string" ? commitBody.json.sha : "";
  if (!commitSha) return { ok: false, code: "provider-error", message: "GitHub commit creation response did not include a sha" };

  // The one irreversible step. No `force` — see this file's header, "KEPT: no force on the ref
  // update" — GitHub rejects this with 422 if the branch moved since `planGitHubFileWrite`'s own tip
  // lookup, which this function reports as `"diverged"`, never silently overwritten.
  const refResult = await githubSend(deps, input.connection, {
    method: "PATCH",
    url: `${repoPath}/git/refs/heads/${encPath(input.branch)}`,
    body: JSON.stringify({ sha: commitSha }),
  });
  if (refResult.kind !== "response") return nonResponseFailure(refResult);
  if (refResult.response.status === 422) {
    return { ok: false, code: "diverged", message: "the branch has moved since this write was confirmed — refusing to overwrite it" };
  }
  if (!isOk(refResult.response)) return { ok: false, code: "provider-error", message: await providerErrorMessage(refResult.response, "GitHub branch update failed") };
