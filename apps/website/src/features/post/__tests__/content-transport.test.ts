/**
 * @file Task 2 of the content-transport (Publish Content) feature — direct unit tests for
 * `contributePostTransport()`/`contributePageTransport()` (`../content-transport.ts`), which
 ...
 * only registry mechanics — see `features/content-transport/__tests__/type-registry.test.ts`), but
 ...
 * `../content-transport.ts`'s own header); the one test for it here just pins that it fails loudly
...
import { contentHash } from "#src/features/content-transport/content-hash";
import { contributePageTransport, contributePostTransport } from "../content-transport.js";
...
// ---------------------------------------------------------------------------
// contributePostTransport / contributePageTransport — data-only contract
// ---------------------------------------------------------------------------

test("contributePostTransport returns data only — entityType/dependsOn are plain values, build is deferred", () => {
  const contributor = contributePostTransport();
...
test("contributePageTransport returns data only, distinguished from post only by entityType", () => {
  const contributor = contributePageTransport();
...
test("both built handlers declare the resource's own content.write permission", () => {
  const deps = makeDeps([]);
  assert.equal(contributePostTransport().build(deps).permission, "content.write");
  assert.equal(contributePageTransport().build(deps).permission, "content.write");
});
...
test("post.pack() yields only kind:'post' rows, each hashed with contentHash('post', ...)", async () => {
  ...
  const handler = contributePostTransport().build(makeDeps([post, page]));
...
test("page.pack() yields only kind:'page' rows", async () => {
  ...
  const handler = contributePageTransport().build(makeDeps([post, page]));
...
test("pack()'s contentHash is stable across two rows with identical content but different id/version/updatedAt", async () => {
  ...
  const handler = contributePostTransport().build(makeDeps([rowA, rowB]));
...
test("inspect() returns {version, hash} for an existing row of the matching kind", async () => {
  ...
  const handler = contributePostTransport().build(makeDeps([post]));
...
test("inspect() returns null for a row that does not exist", async () => {
  const handler = contributePostTransport().build(makeDeps([]));
...
test("inspect() returns null for a row that exists but is the WRONG kind — a page's id must not resolve through the post handler", async () => {
  const page = makePost({ id: "page-1", kind: "page", slug: "a-page" });
  const handler = contributePostTransport().build(makeDeps([page]));
...
test("precheck() allows an entity claiming ITS OWN current slug (updating in place is not a collision)", async () => {
  ...
  const handler = contributePostTransport().build(makeDeps([existing]));
  const result = await handler.precheck({
    entityType: "post",
    id: "post-1",
    contentHash: "irrelevant",
