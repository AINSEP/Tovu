 ...
 * `contributePostTransport()` handler (Task 2) plus a real `InMemoryPostRepo`, through the REAL
 * registry (`registerContentTransportContributor`), so this planner is shown to actually compose
...
function fakeContributor(handler: ContentTransportHandler): ContentTransportContributor {
...
/** Builds `PlanImportDeps` backed by plain maps/sets — no real `ContentTransportDeps` is needed
...
function makeDeps(options: {
  ...
  return {
    contentTransportDeps: {} as PlanImportDeps["contentTransportDeps"],
...
test("created: no destination row for the id", async () => {
  ...
  registerContentTransportContributor(fakeContributor(makeFakeHandler({ entityType: "widget", destination })));
  const before = snapshot(destination);

  const bundle: TransportBundle = { hashVersion: CONTENT_HASH_VERSION, entities: [makeEntity({ entityType: "widget", id: "w1" })] };
...
test("blocked: precheck fails (e.g. slug taken) — never caught as a write-time constraint violation", async () => {
  ...
  registerContentTransportContributor(fakeContributor(handler));
  const before = snapshot(destination);
...
test("blocked: an entity type present in the bundle with NO registered handler on this instance", async () => {
  // No contributor registered at all for "widget" — resetContentTransportContributorsForTests()
  // ran in beforeEach.
  const entity = makeEntity({ entityType: "widget", id: "w1" });
  const report = await planImport({ hashVersion: CONTENT_HASH_VERSION, entities: [entity] }, makeDeps({}));

  assert.equal(report.rows[0].outcome, "blocked");
  assert.match(report.rows[0].reason ?? "", /no registered content-transport handler for entity type 'widget'/);
});
...
test("planImport's applyOrder places a dependency's type before its dependent's, and rows follow that order", async () => {
  ...
  registerContentTransportContributor(fakeContributor(makeFakeHandler({ entityType: "media", destination: mediaDestination })));
  registerContentTransportContributor(
...
test("real post contributor: created + byte-identical destination", async () => {
  ...
  const { contributePostTransport } = await import("../../post/content-transport.js");
  ...
  const deps: PlanImportDeps = {
    contentTransportDeps: { workspaceId, postRepo: repo, clock: { nowIso: () => "2026-09-18T00:00:00.000Z" }, idGen: { newId: () => "unused" } },
