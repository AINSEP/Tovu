...
// `content-transport` server module (which would drag in every route's Express wiring just for
 ...
 *
 * `content.transport.read`/`content.transport.apply` are granted directly to the built-in `admin`
...
const CONTENT_TRANSPORT_READ = "content.transport.read";
const CONTENT_TRANSPORT_APPLY = "content.transport.apply";
...
function copyRealContentDbToTempDir(): { readonly dir: string; readonly dbPath: string } {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "content-transport-perm-boot-test-"));
...
test("booting against a COPY of the real, already-seeded content.db grants content.transport.read and content.transport.apply to admin", async () => {
  ...
  try {
    ...
    assert.ok(
      adminPermissions.includes(CONTENT_TRANSPORT_READ),
      "admin must hold content.transport.read after boot reconciles the real content.db"
    );
    assert.ok(
      adminPermissions.includes(CONTENT_TRANSPORT_APPLY),
      "admin must hold content.transport.apply after boot reconciles the real content.db"
    );
  } finally {
...
test("editor and viewer do NOT gain content.transport.read or content.transport.apply", async () => {
  const { dir, dbPath } = copyRealContentDbToTempDir();
  try {
    const deps = await bootAgainstRealContentDbCopy(dbPath, "boot");
    ...
    for (const policyName of ["editor-builtin-policy", "viewer-builtin-policy"]) {
      ...
      assert.ok(
        !permissions.includes(CONTENT_TRANSPORT_READ),
        `${policyName} must NOT gain content.transport.read`
      );
      assert.ok(
        !permissions.includes(CONTENT_TRANSPORT_APPLY),
        `${policyName} must NOT gain content.transport.apply`
      );
    }
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("re-running boot against the same already-migrated copy is idempotent — no duplicate grant rows", async () => {
  ...
  try {
    ...
    const readCount = adminPermissions.filter((permission) => permission === CONTENT_TRANSPORT_READ).length;
    const applyCount = adminPermissions.filter((permission) => permission === CONTENT_TRANSPORT_APPLY).length;

    assert.equal(readCount, 1, "re-running boot must not duplicate the content.transport.read row");
    assert.equal(applyCount, 1, "re-running boot must not duplicate the content.transport.apply row");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
