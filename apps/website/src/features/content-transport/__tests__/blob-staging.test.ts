...
import {
  bytesMatchSha256,
  CONTENT_TRANSPORT_BLOB_PROBE_MAX_SHAS,
  isValidSha256Hex,
  sha256Hex,
...
test("sha256Hex hashes bytes to their lowercase hex digest", () => {
  const bytes = new TextEncoder().encode("hello content-transport");
...
test("CONTENT_TRANSPORT_BLOB_PROBE_MAX_SHAS is a real positive bound, not accidentally zero/Infinity", () => {
  assert.ok(Number.isFinite(CONTENT_TRANSPORT_BLOB_PROBE_MAX_SHAS));
  assert.ok(CONTENT_TRANSPORT_BLOB_PROBE_MAX_SHAS > 0);
});
