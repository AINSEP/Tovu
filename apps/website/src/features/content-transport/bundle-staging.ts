 ...
 * `ADS-memory/reports/2026-09-18-publish-feature-implementation-plan.md` §2 (`content_transport_
 * bundles`) / §4 task 6.
 *
 ...
 * schema.ts`'s `contentTransportBundles`). Bytes are NOT stored here — see `blob-staging.ts`.
 *
 ...
 *
 * **Deviation from the plan's draft schema (disclosed):** plan §2's `content_transport_bundles`
 * draft included a `sourceLabel`/label-style column; the migration Task 3 actually landed
 * (`platform/db/schema.ts`'s `contentTransportBundles`, migration `0066`) has NO such column — only
 ...
 * `contentTransportRuns.peerLabel`'s own doc, "never used in any comparison or trust decision", so
 * dropping it here costs nothing safety-relevant. Task 7's `planImport` already treats
 * `TransportBundle.sourceLabel` as optional. Not fixed by editing the migration: out of this task's
...
/** One staged bundle row — the in-process shape `ContentTransportBundleRepoPort` reads/writes.
 *  Mirrors `contentTransportBundles` (`platform/db/schema.ts`) field-for-field; `sourceLabel` is
...
export interface StagedBundleRecord {
   ...
   *  Mirrors `content_transport_baselines.peerPrincipalId`'s own rule (plan §1.6 / §5 risk #9): a
...
export interface ContentTransportBundleRepoPort {
 ...
 */
export const CONTENT_TRANSPORT_BUNDLE_TTL_MS = 24 * 60 * 60 * 1000;

/**
 * Body-size ceiling for `POST .../content-transport/bundles`, mirroring `rejectOversizedJsonBody`'s
 ...
 */
export const CONTENT_TRANSPORT_BUNDLE_MAX_BODY_BYTES = 20 * 1024 * 1024;
...
export function computeBundleExpiry(required: { receivedAt: string; ttlMs?: number }): string {
  const ttlMs = required.ttlMs ?? CONTENT_TRANSPORT_BUNDLE_TTL_MS;
...
export interface StageBundleDeps {
  readonly repo: ContentTransportBundleRepoPort;
  readonly clock: { nowIso(): string };
  readonly idGen: { newId(): string };
  /** Test-only override of {@link CONTENT_TRANSPORT_BUNDLE_TTL_MS} — absent in real composition. */
...
export async function loadActiveBundle(
  required: { repo: ContentTransportBundleRepoPort; workspaceId: string; id: string; now: string }
...
/** In-process `ContentTransportBundleRepoPort` — hermetic tests and any dev/test composition that
...
export class InMemoryContentTransportBundleRepo implements ContentTransportBundleRepoPort {
