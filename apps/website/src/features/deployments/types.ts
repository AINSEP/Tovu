 ...
 * (`src/platform/db/schema.sqlite.ts`'s `deploymentEnvironments`/`deploymentTargets`/`releases`/`deploymentRuns`/
...
export interface DeploymentRunRecord {
   ...
   * (`src/platform/db/schema.sqlite.ts`) rather than either a hard restrict or no FK at all: creation is still
