...
| Container packaging | ~~No `Dockerfile`…~~ **PARTLY STALE — corrected 2026-08-31.** A real `Dockerfile`, `Dockerfile.dockerignore` and `docker-compose.yml` now exist at the repo root. Still absent: `fly.toml`, `render.yaml`. The Dockerfile's own header is the authoritative note that its build context must be the PARENT directory — Tovu declares 22 `file:` deps (12 root + 10 `apps/admin`) into `../Jini/packages/*`, and Jini's internal deps use pnpm `workspace:*`, which rules out `npm pack`/`--install-links` workarounds. **Consequence: git-based deploy on Railway/Render cannot work** — both clone a single repo and can't see Jini. Build locally, push an image, deploy the image. |
