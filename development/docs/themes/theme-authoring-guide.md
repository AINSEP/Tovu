...
*Correction to a starting assumption:* the task that produced this doc assumed `fuel`/Ember is *currently* the active theme. As of this session, the local dev database (`infra/content.db`, `presentation_settings` table) actually has `activeThemeId = "basic"` — not `fuel`. `basic` does wire the menu embed. `tovu-official`, the hardcoded seed default in `src/server/seed.ts:255`, refers to a theme that only exists in the dead `src/theme-archive/` tree and is not discoverable at all (§8.1) — on a fresh workspace this default fails to resolve, and `resolveActiveTheme()`'s fallback (§1.2) picks the alphabetically-first *valid* discovered theme, which is `basic`. Whichever theme is active is admin-mutable at any time; don't treat "the active theme" as a fixed fact — check `presentation_settings` (or the admin UI) for ground truth. The `fuel`-hardcodes-its-nav fact itself is independent of which theme happens to be active and remains true regardless.
...
### 8.1 The seed default (`tovu-official`) points at a theme that no longer exists

`src/server/seed.ts:255` hardcodes `activeThemeId: "tovu-official"` as the default for a brand-new workspace. `tovu-official` lives only under `src/theme-archive/` (`src/theme-archive/tovu-official/`), which `builtInThemesDir()` never scans (`src/server/deps.ts:143-145` resolves to `src/themes/`, not `src/theme-archive/`). On a fresh workspace this default will not resolve to a real theme; `resolveActiveTheme()`'s fallback then silently picks the alphabetically-first valid discovered theme instead (currently `basic`). The seed value isn't wrong in a way that crashes anything — the fallback papers over it completely — but it is stale and should not be read as "the intended default theme."
