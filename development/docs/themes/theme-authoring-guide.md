...
| `templates` (renamed from `postTemplate`/`pageTemplate`, unified 2026-08-11 — `theme.ts:196-207`) | string[] | `static` only | no | Ordered list of `pages/*.html` filenames a Post **or a Page** can pick between via `templateChoice` (§7.1, §7.3). One array for both kinds — a template file no longer declares which kind it's "for" (`theme.ts:197`, `static-render.ts:72`). First entry is the implicit default. Each entry is validated against the theme's actual pages by `validateTemplateDeclarations` (`theme.ts:615-641`): it must resolve to a real `pages/<id>.html` file containing at least one `{"type":"content"}` marker, or the theme fails to load. |
...
### 7.1 `templateChoice` — which static-theme page template a Post (or Page) renders through

`templateChoice: string | null` (`src/features/post/post.ts:97`, DB column `template_choice`, `src/platform/db/schema.sqlite.ts:107`) is a **tri-state**, and the difference between two of its values is the entire point. The column lives on the shared `posts` table (`kind: "post" | "page"`), so everything below applies to Pages exactly as it applies to Posts — despite the field's own name and this section's title, which predate the 2026-08-11 unification that generalized it (§3.1's `templates` row, `theme.ts:197`) — **except the `""` row**, which diverges by `kind` since the bare-page ruling (2026-09-23).

| Value | Meaning | Render result |
|---|---|---|
| `null` / absent | *Never chosen* — the pre-feature default, and what every row written by any path unaware of this field (seed script, agent tool, direct insert) reads as | Falls back to the theme's `templates[0]` (first-listed template) |
| `""` (empty string), Post | *Explicitly opted out* via the admin picker's "No template chosen" option | The diagnostic "Template not configured" page (`buildMissingPostTemplateHtml`, `pages.ts:246-269`) — deliberately not a silent fallback |
| `""` (empty string), Page | *Explicitly opted out* via the admin picker's "No template — HTML only" option (bare-page ruling, 2026-09-23) | **Bare**: only the Page's own HTML, resolved and with embeds/SEO applied, but no theme CSS/JS, no `data-theme`, no header/footer, no site-assistant widget (`isBarePageChoice`, `static-render.ts`; `renderBarePage`, `pages.ts`) — a different outcome from a Post's `""`, on purpose |
| `"blog-post.html"` (a real filename) | An explicit choice | Rendered as-is **if the currently active theme can honor it** (has a page by that name, and that page has a `data-embed-id="{{post}}"` slot) — otherwise treated exactly like the `null` case, i.e. falls back to `templates[0]`, *not* to the diagnostic page |
...
This whole path only runs when the active theme is `static` **and** declares a non-empty `templates` array in `theme.json` (`pages.ts:532`). A theme with no `templates` array simply doesn't support this feature — those posts (and pages) fall through to the ordinary `renderSite()` post path, unconditionally.

**Nothing here checks that the chosen template's filename "matches" the row's `kind`.** A Page can set `templateChoice` to a Post-designed template and vice versa — see §7.3 for why that's deliberate, not a gap.

### 7.2 `overridesThemePage` — slug-collision tiebreak between a Post and a theme's own marketing page

`overridesThemePage: boolean` (default `false`, `src/platform/db/schema.sqlite.ts:117`) only matters when a real Post's slug happens to collide with one of the active static theme's own `pages/*.html` filenames (e.g. a post at slug `about` colliding with `pages/about.html`). By default, **the theme's page wins** — it's a reserved, reliable namespace. Setting `overridesThemePage: true` on the specific post (an explicit author action, made after the admin UI warns about the collision) flips that one post to win instead (`pages.ts:497-521`, schema comment `db/schema.sqlite.ts:108-116`).
...
### 7.3 Content-template naming convention — `posts-*` / `pages-*` (descriptive, not enforced)

**The rule:** a `theme.json` `templates` entry (§3.1, §7.1) is named for the content kind it was designed for — `posts-*.html` for a template designed with Posts in mind, `pages-*.html` for one designed with Pages in mind (e.g. `posts-default.html`, `posts-sidebar.html`, `pages-default.html`). It applies only to files listed in `templates`; a static theme's standalone route pages (`index.html`, `about.html`, `blog.html`, `docs.html`, `404.html`, and similar) are addressable routes, not content templates, and keep their existing names regardless of this rule.

**The prefix is descriptive of design intent only — it is not enforced.** Nothing in `validateTemplateDeclarations` (`theme.ts:615-641`), `loadTheme()`, or the render path (`resolvePostTemplate`, `static-render.ts:278-297`) checks a template's filename against the `kind` of the row selecting it via `templateChoice`, so a Post may still select a `pages-*` template and a Page may still select a `posts-*` template, and both render exactly as chosen with no warning or error. This is not an oversight to fix — §7.1 already establishes that nothing here checks kind-vs-template agreement, and that is a direct continuation of the 2026-08-11 unification's own design: the old kind-scoped `postTemplate`/`pageTemplate` split existed only because the retired marker vocabulary made a template's slot type kind-specific, and it was retired precisely because that let a Post-shaped template reach a Page (or vice versa) anyway (`theme.ts:197`, `static-render.ts:72`). Cross-kind selection is already live, observed production usage, not a hypothetical: on `sites/tovu-com/content.db`, `blog-post.html` and `blog-sidebar-template.html` (both Post-prefixed under this convention) are each selected by 3 `kind: "page"` rows today. See `ADS-memory/reports/architecture/ADR-065-content-template-naming-convention.md` for the full evidence and the reasoning for leaving this unenforced.

**One filename in `templates` is not a free author choice.** The theme's generic document shell — the file `renderStaticTierHomePage`'s sibling fallback looks up for any `html`-format Page with no template chosen — is a **Tovu-owned, closed-vocabulary filename**, read by two hardcoded constants that never consult `theme.manifest.templates` or its ordering: `STATIC_TIER_PAGE_SHELL_ID` (`static-render.ts` — re-check the line number, it moved to `:792` as of the 2026-09-03 rename work, from `:755` before it) and `STATIC_TIER_PAGE_SHELL_TEMPLATE` (`apps/admin/src/features/pages/hooks/use-theme-canvas-styling.hooks.ts:77`). Under this convention that file is named `pages-default.html` (see below for the legacy name it replaces). A theme that names this file anything else simply has no page shell: an untemplated `html`-format Page degrades to Tovu's generic chrome (no `data-theme`, no theme scripts/icons/manifest) instead of the theme's own shell — a silent-looking but logged failure mode (`static-render.ts`'s own `[theme] …` warning, once per distinct message), not a hard error. **As last observed (2026-09-03, uncommitted working-tree state), only the `resolveTemplate`/`templateChoice` path has a legacy-name compatibility shim (`LEGACY_TEMPLATE_FILENAME_ALIASES`, below) — `STATIC_TIER_PAGE_SHELL_ID` itself was still the literal string `"page-shell"`, not alias-aware, which on a theme that has renamed the file and ships nothing at the old name would miss and degrade Pages to generic chrome. Re-verify this against current `HEAD` before relying on it; it may already be fixed.**

**Adoption is partial, deliberately.** As last observed (2026-09-03, uncommitted working-tree state), the live site copy of the `basic` theme (`sites/tovu-com/themes/static/basic/`) has been renamed to this convention — its `theme.json` `templates` array and `render/pages/` directory both carry the new names:
...
The shipped seed theme (`content/themes/static/basic/`), plus `basic-2`, `tailark-dusk`, `tailark-quartz-dark`, `tailark-quartz-libre`, and `__original-themes__`, still ship the pre-convention names as of this writing — confirmed directly against each theme's own `theme.json` `templates` array, not assumed. **Legacy names keep resolving.** `static-render.ts`'s `resolveTemplate` carries a `LEGACY_TEMPLATE_FILENAME_ALIASES` compatibility map (old filename id → new id, consulted only as a fallback after a direct lookup misses) so a stored `templateChoice` naming an old filename keeps resolving correctly, on the renamed theme and on every theme that hasn't renamed. **Stored `template_choice` values are deliberately not backfilled to the new names** — the 27 rows on `sites/tovu-com/content.db` still storing `blog-post.html`/`blog-sidebar-template.html`/`page-shell.html` (observed 2026-09-03) stay as-is by design: rewriting them to the new name would make a row *more* fragile on a future theme switch (a still-unrenamed theme would stop matching a backfilled row), not less. See the ADR for the full reasoning and evidence.
...
This alone is a valid, renderable theme. To add a marketing page, drop another file in `pages/` (e.g. `pages/about.html`) and it becomes reachable at `/about` automatically (`pages.ts:504`, §1.4 step 4) — no manifest entry needed for it to render, though listing it in `theme.json`'s `pages` array is the existing-theme convention even though nothing reads it (§3.2). To support the post-template picker, add a page with a `data-embed-id="{{post}}"` marker (§6.4/§7.1) and list its filename in `theme.json`'s `templates` array, named per the §7.3 convention (e.g. `posts-default.html`).
...
- `ADS-memory/reports/architecture/ADR-065-content-template-naming-convention.md` — the `posts-*`/`pages-*` template-naming decision (§7.3 above): full evidence, why it's descriptive rather than enforced, and the partial-adoption state.
- `development/docs/themes/theme-authoring-guide-v2.md` — the settled-but-not-fully-built schema v2 target design (`render/pages/`, `apiVersion`, structured `authors`/`license`). Its own banner tells you to read *this* document for real authoring work; treat v2 as the machine-checkable field reference, not the entry point.
