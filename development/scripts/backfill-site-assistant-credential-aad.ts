 ...
 * only what's genuinely specific to `site_assistant_credentials`: its identity shape, its AAD
...
function loadPendingUnits(db: ContentDb): AadBackfillUnit[] {
  return loadPendingRows(db).map((row) => ({
    ...
    sealed: row.sealed,
    buildAad: () => buildSiteAssistantCredentialAad({ workspaceId: row.workspaceId }),
    write: (sealed) =>
      db
        ...
        .where(and(eq(siteAssistantCredentials.workspaceId, row.workspaceId), eq(siteAssistantCredentials.aadVersion, 0)))
        .run().changes,
  }));
}
...
const messages: AadBackfillMainMessages = {
  ...
  mismatch: (label) =>
    `site-assistant-credential AAD backfill: post-seal verification mismatch for ${label} — refusing to write a row that cannot be proven to re-open correctly`,
  ...
  nothingToMigrate: () => "Nothing to migrate — every site_assistant_credentials row with a key already carries aad_version=1.",
  done: (result) => `Done: ${result.migrated} row(s) migrated, ${result.total} total pending.`,
};

export interface SiteAssistantCredentialAadBackfillDeps extends AadBackfillDeps {}
export interface SiteAssistantCredentialAadBackfillResult extends AadBackfillResult {}
...
async function main(): Promise<void> {
  await runAadBackfillMain(process.argv.slice(2), {
    ...
    restorePointScopeId: "backfill-site-assistant-credential-aad",
