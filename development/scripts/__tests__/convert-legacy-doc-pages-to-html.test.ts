...
/**
 * @file Data-safety proof for `convert-legacy-doc-pages-to-html.ts` — the two guards every sibling
...
const SCRIPT = path.join("development", "scripts", "convert-legacy-doc-pages-to-html.ts");
...
test("convert-legacy-doc-pages-to-html: a dry run against a not-yet-migrated content.db must not migrate it — asserted on the actual file, not a log line", () => {
  const scratch = tmpDir("convert-legacy-doc-pages-nomigrate-");
...
test("convert-legacy-doc-pages-to-html: a mistyped --db path fails loudly and creates nothing, instead of silently opening an empty database", () => {
  const scratch = tmpDir("convert-legacy-doc-pages-missingdb-");
  ...
  // every allowlisted slug as "NOT FOUND" instead of the real problem — no such database.
  assert.doesNotMatch(stderr, /NOT FOUND/);
