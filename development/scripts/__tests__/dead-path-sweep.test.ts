...
const KNOWN_BROKEN_PENDING_OWNER_DECISION: Readonly<Record<string, KnownBrokenEntry>> = {
  ...
  ...known(
    ...
    `${UNRUN_ONE_SHOT} Note this script builds its own ROOT as development/, not the repo root, so ` +
      "the true dead target is development/src/platform/db/schema.sqlite.ts — absent under either base.",
...
test("historical: the pre-7fb47f55 drizzle.config.ts path strings are flagged as dead", () => {
  ...
  assert.deepEqual(paths, ["src/platform/db/schema.sqlite.ts", "src/platform/db/drizzle"]);
...
test("classifyRepoRelativeString names the rule for every string it declines", () => {
  ...
  const cases: readonly [string, string][] = [
    ...
    ["#src/platform/db/schema.sqlite.js", "package-imports-specifier"],
    ...
    ["schema.sqlite.ts", "no-path-separator"],
...
test("classifyRepoRelativeString hands a `./` literal that is also an import specifier back to class 1", () => {
  ...
  const withoutHint = classifyRepoRelativeString("./apps/website/src/platform/db/schema.sqlite.ts", {
    knownRepoSegments: segments,
  });
  assert.deepEqual(withoutHint, { kind: "repo-relative-path", repoRelative: "apps/website/src/platform/db/schema.sqlite.ts" });
});
...
test("pathThatMustExist requires source files whole and non-source paths only down to their directory", () => {
  assert.equal(pathThatMustExist("apps/website/src/platform/db/schema.sqlite.ts"), "apps/website/src/platform/db/schema.sqlite.ts");
